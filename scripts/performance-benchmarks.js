...
class PerformanceBenchmark {
  ...
  async runBenchmark() {
    ...
    const configs = [
      {
        name: "performance",
        config: "jest.config.performance.cjs",
        target: 10000,
      },
    ...
    for (const { name, config, target } of configs) {
      ...
      try {
        ...
        const result = execSync(
          `npm test -- --config=${config} --passWithNoTests --silent`,
          {
            encoding: "utf8",
        ...
        console.log(
          `  📈 Improvement: ${results.configurations[name].improvement}\n`,
        );
      } catch (error) {
        console.error(
          `  ❌ Failed to run ${name} configuration:`,
          error.message,
        );
        results.configurations[name] = {
          error: error.message,
  ...
  async compareWithBaseline(results) {
    ...
    for (const [configName, config] of Object.entries(results.configurations)) {
      ...
      if (!baselineConfig || !baselineConfig.duration || !config.duration)
        continue;

      const improvement = baselineConfig.duration - config.duration;
      const improvementPercent = (
        (improvement / baselineConfig.duration) *
        100
      ).toFixed(1);
      ...
      console.log(
        `  Change: ${improvement > 0 ? "✅" : "❌"} ${improvementPercent}% ${improvement > 0 ? "faster" : "slower"}\n`,
      );
    }
  }
  ...
  generateReport(results) {
    ...
    console.log(
      `🖥️  System: ${results.platform} | ${results.cpus} CPUs | ${results.memory}`,
    );
    ...
    sortedConfigs.forEach(([name, config], index) => {
      ...
      console.log(
        `${emoji} ${rank}. ${name.toUpperCase()}: ${config.duration}ms ${status}`,
      );
      console.log(
        `     Target: ${config.target}ms | Improvement: ${config.improvement}`,
      );

      if (config.tests) {
        console.log(
          `     Tests: ${config.tests.passed}/${config.tests.total} passed | ${config.testSuites.passed}/${config.testSuites.total} suites\n`,
        );
      }
    });
    ...
    if (fastestConfig && fastestConfig[1].duration <= 10000) {
      ...
      console.log(
        `   Fastest configuration: ${fastestConfig[0]} (${fastestConfig[1].duration}ms)`,
      );
    } else {
...
async function main() {
  ...
  try {
    ...
    if (
      performanceConfig &&
      performanceConfig.duration &&
      performanceConfig.duration <= 10000
    ) {
