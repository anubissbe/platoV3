...
function analyzeProjectFiles() {
  ...
  allFiles.forEach(filePath => {
    ...
    // Categorize files
    if (filePath.includes('__tests__') || filePath.includes('__mocks__') || fileName.includes('.test.') || fileName.includes('.spec.')) {
      analysis.categories.test_files.push(filePath);
    } else if (fileName.endsWith('.d.ts') || fileName === 'types.ts' || fileName === 'interfaces.ts') {
      analysis.categories.type_files.push(filePath);
    } else if (fileName === 'constants.ts' || fileName === 'config.ts' || fileName.includes('.config.') || fileName.includes('.constants.')) {
    ...
    } else if (filePath.includes('/generated/') || fileName.includes('.generated.')) {
      analysis.categories.generated_files.push(filePath);
    } else if (fileName === 'cli.ts' || fileName === 'main.ts' || fileName === 'app.ts') {
      analysis.categories.entry_points.push(filePath);
    } else if (filePath.includes('/vendor/') || filePath.includes('/third-party/') || filePath.includes('/external/')) {
      analysis.categories.vendor_files.push(filePath);
    } else if (filePath.includes('/docs/') || filePath.includes('/examples/') || fileName.includes('.example.') || fileName.includes('.stories.')) {
  ...
  if (analysis.categories.small_files.length > 0) {
    analysis.recommendations.push({
      ...
      message: 'Consider excluding very small files (< 50 bytes) as they likely contain only exports or constants',
  ...
  if (analysis.categories.index_files.length > 10) {
    analysis.recommendations.push({
      ...
      message: 'Many index files detected - these are typically re-exports and can be excluded',
 ...
 */
function updateJestConfig(exclusionPatterns, configPath = 'jest.config.coverage.cjs') {
  ...
  const exclusionPatternsFormatted = exclusionPatterns.map(pattern => `'${pattern}'`);
...
function generateExclusionReport(analysis, exclusionPatterns) {
  ...
  Object.entries(analysis.categories).forEach(([category, files]) => {
    if (Array.isArray(files) && files.length > 0) {
      console.log(`${category.replace('_', ' ').padEnd(20)}: ${files.length} files`);
    }
  });
...
function main() {
  try {
    ...
    const analysisPath = path.join(process.cwd(), 'coverage-exclusion-analysis.json');
