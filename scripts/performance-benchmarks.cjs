...
class BenchmarkResult {
  ...
  evaluatePerformance(pathId) {
    ...
    if (value <= path.target) {
      return {
        status: "excellent",
        message: `Within target (${path.target}ms)`,
      };
    } else if (value <= path.warning) {
      return {
        status: "good",
        message: `Acceptable (${path.warning}ms threshold)`,
      };
    } else if (value <= path.critical) {
      return {
        status: "warning",
        message: `Approaching critical threshold (${path.critical}ms)`,
      };
    } else {
      return {
        status: "critical",
        message: `Exceeds critical threshold (${path.critical}ms)`,
      };
    }
  }
}
...
class PerformanceBenchmarkRunner {
  ...
  async benchmarkPath(pathId, benchmarkFunction) {
    ...
    if (this.options.verbose) {
      ...
      console.log(
        `   Target: ${pathConfig.target}ms, Warning: ${pathConfig.warning}ms, Critical: ${pathConfig.critical}ms`,
      );
    }
    ...
    if (this.options.verbose) {
      console.log(
        `   Warming up (${this.options.warmupIterations} iterations)...`,
      );
    }
    ...
    if (this.options.verbose) {
      console.log(
        `   Running benchmark (${this.options.iterations} iterations)...`,
      );
    }
    ...
    for (let i = 0; i < this.options.iterations; i++) {
      try {
        ...
        if (this.options.verbose) {
          const statusIcon =
            duration <= pathConfig.target
              ? "✅"
              : duration <= pathConfig.warning
                ? "⚠️"
                : "❌";
          process.stdout.write(
            `   ${statusIcon} Iteration ${i + 1}: ${duration.toFixed(1)}ms\n`,
          );
        }
      } catch (error) {
  ...
  printResult(pathId, result) {
    ...
    console.log(
      `      Min/Max:   ${stats.min.toFixed(1)}ms / ${stats.max.toFixed(1)}ms`,
    );
    ...
    console.log(
      `   ${statusIcons[perf.status]} Status: ${perf.status.toUpperCase()} - ${perf.message}`,
    );
  }
...
Examples:
  ...
  try {
    ...
    console.log(
      `Results: ${report.summary.statusCounts.excellent}🟢 ${report.summary.statusCounts.good}✅ ${report.summary.statusCounts.warning}⚠️ ${report.summary.statusCounts.critical}❌`,
    );
