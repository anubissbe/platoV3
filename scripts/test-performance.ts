...
async function runTestConfig(config: string, description: string): Promise<TestResult> {
  console.log(`\n🧪 Running ${description}...`);
...
async function main() {
  ...
  for (const config of configs) {
    try {
      ...
      console.log(`   Tests: ${result.passedTests}/${result.totalTests} passed, ${result.failedTests} failed`);
  ...
  if (results.length >= 2) {
    const baseline = results.find(r => r.config === 'test') || results[results.length - 1];
    ...
    if (fastest !== baseline) {
      const improvement = Math.round(((baseline.duration - fastest.duration) / baseline.duration) * 100);
      const speedup = Math.round(baseline.duration / fastest.duration * 10) / 10;
    ...
    if (fastest.config === 'test:fast') {
      ...
      console.log('   ✓ Use `npm run test:integration-only` for integration tests');
    }
    
    console.log('\n📈 Test Suite Health:');
    results.forEach(result => {
      const passRate = Math.round((result.passedTests / result.totalTests) * 100);
      const icon = passRate >= 90 ? '🟢' : passRate >= 70 ? '🟡' : '🔴';
      console.log(`   ${icon} ${result.config}: ${passRate}% pass rate (${result.totalTests} tests)`);
    });
  }
}
