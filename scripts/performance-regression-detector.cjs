...
class PerformanceRegressionDetector {
  constructor(options = {}) {
    ...
    this.baselineDir = path.join(process.cwd(), this.config.baseline.storageDir);
  ...
  async analyzePerformance(currentResults, options = {}) {
    ...
    for (const [pathId, currentResult] of Object.entries(currentResults)) {
      ...
      try {
        const pathAnalysis = await this.analyzePathPerformance(pathId, currentResult);
        ...
        if (pathAnalysis.regression.detected) {
          ...
          const severityLevels = { none: 0, minor: 1, moderate: 2, major: 3, critical: 4 };
          if (severityLevels[pathAnalysis.regression.severity] > severityLevels[analysis.severity]) {
          ...
          if (pathAnalysis.regression.severity !== 'minor') {
            analysis.recommendations.push({
              ...
              action: this.getRecommendedAction(pathAnalysis.regression.severity),
  ...
  async analyzePathPerformance(pathId, currentResult) {
    ...
    const pathAnalysis = {
      ...
      regression: { detected: false, severity: 'none', description: '', impact: 0 },
      trend: { direction: 'stable', confidence: 0 }
    };
    
    if (baselines.length < this.config.analysis.minimumSamples) {
      console.log(`📊 ${pathId}: Insufficient baselines (${baselines.length}/${this.config.analysis.minimumSamples})`);
  ...
  analyzeChange(currentValue, baselineStats) {
    ...
    if (confidence >= 0.8) { // High confidence threshold
      if (ratio > 0.05) { // 5% threshold for degradation
        change.type = 'degradation';
      } else if (ratio < -0.05) { // 5% threshold for improvement
  ...
  analyzeTrend(baselines, currentValue) {
    ...
    if (trend.confidence > 0.7) { // High confidence threshold
      if (slope > 5) { // 5ms per measurement threshold
  ...
  calculateConfidence(zScore) {
    ...
    if (absZ >= 1.64) return 0.90;  // 90% confidence
    if (absZ >= 1.28) return 0.80;  // 80% confidence
    if (absZ >= 1.04) return 0.70;  // 70% confidence
  ...
  async loadBaselines(pathId) {
    ...
    cutoffDate.setDate(cutoffDate.getDate() - this.config.baseline.retentionDays);
  ...
  printPathAnalysis(pathId, analysis) {
    const statusIcon = analysis.regression.detected 
      ? (analysis.regression.severity === 'critical' ? '🚨' : 
         ...
         analysis.regression.severity === 'moderate' ? '⚠️' : '⚡')
    ...
    if (analysis.regression.detected) {
      ...
      console.log(`   📊 Confidence: ${(analysis.regression.confidence * 100).toFixed(1)}%`);
    } else if (analysis.change.type === 'improvement') {
      console.log(`   📈 Improved by ${(analysis.change.magnitude * 100).toFixed(1)}%`);
    } else {
      console.log(`   ✅ Performance stable`);
    }
    
    if (analysis.trend.confidence > 0.7) {
      const trendIcon = analysis.trend.direction === 'degrading' ? '📉' : 
                       analysis.trend.direction === 'improving' ? '📈' : '➡️';
      console.log(`   ${trendIcon} Trend: ${analysis.trend.direction} (${(analysis.trend.confidence * 100).toFixed(0)}% confidence)`);
    }
  }
  ...
  generateOverallAssessment(analysis) {
    ...
    if (summary.regressions > 0) {
      ...
      if (analysis.severity === 'critical' || regressionRate > 0.5) {
        status = 'critical';
        message = 'Critical performance regressions detected - immediate action required';
        ...
        status = 'warning';
        message = 'Some performance degradation detected - monitoring recommended';
      } else {
        status = 'minor_issues';
...
Examples:
  ...
  const options = {
    analysis: {
      confidenceLevel: parseFloat(args[args.indexOf('--confidence') + 1]) || 0.95
    },
    baseline: {
  ...
  try {
    switch (command) {
      case 'analyze': {
        ...
        const analysis = await detector.analyzePerformance(reportData.results || reportData);
        
        // Save analysis results
        const analysisPath = reportFile.replace('.json', '-regression-analysis.json');
        ...
        console.log(`   Results: ${analysis.summary.regressions}⚠️ ${analysis.summary.improvements}📈 ${analysis.summary.stable}✅`);
