...
async function verifyMCPFixes() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        '✅ Permission system initializes without "No active profile" error',
      );
      await detachServer("test-permission-system");
    } catch (error) {
      if (error.message.includes("No active profile")) {
        throw new Error(
          "CRITICAL: Permission system not properly initialized - " +
            error.message,
        );
      }
    ...
    if (healthResults.length !== 2) {
      throw new Error(
        `Expected health results for 2 servers, got ${healthResults.length}`,
      );
    }
    ...
    console.log(
      `   Unhealthy servers: ${unhealthyCount} (expected for fake URLs)`,
    );
    ...
    for (const serverTools of toolsList) {
      console.log(
        `   Server ${serverTools.server}: ${serverTools.tools.length} tools`,
      );
    }
    ...
    if (servers3.length !== 0) {
      throw new Error(
        `Expected 0 servers after cleanup, got ${servers3.length}`,
      );
    }
