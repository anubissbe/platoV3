...
function main() {
  try {
    ...
    const badges = {
      lines: {
        ...
        url: generateBadgeUrl(
          "Coverage-Lines",
          `${totals.lines.pct}%`,
          getBadgeColor(totals.lines.pct),
        ),
      },
      statements: {
        ...
        url: generateBadgeUrl(
          ...
          getBadgeColor(totals.statements.pct),
        ),
      },
      functions: {
        ...
        url: generateBadgeUrl(
          ...
          getBadgeColor(totals.functions.pct),
        ),
      },
      branches: {
        ...
        url: generateBadgeUrl(
          "Coverage-Branches",
          `${totals.branches.pct}%`,
          getBadgeColor(totals.branches.pct),
        ),
      },
    };
    ...
    Object.entries(badges).forEach(([type, badge]) => {
      const typeLabel = type.charAt(0).toUpperCase() + type.slice(1);
      console.log(
        `[![${typeLabel} Coverage](${badge.url})](./coverage/lcov-report/index.html)`,
      );
    });

    // Save badge data for programmatic use
    const badgeDataPath = path.join(
      process.cwd(),
      "coverage",
      "badge-data.json",
    );
    ...
    Object.entries(badges).forEach(([type, badge]) => {
      if (badge.percentage < thresholds[type]) {
        failedThresholds.push(
          `${type}: ${badge.percentage}% < ${thresholds[type]}%`,
        );
      }
    });
