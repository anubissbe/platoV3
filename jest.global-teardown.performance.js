...
module.exports = async () => {
  ...
  if (fs.existsSync(reportsDir)) {
    ...
    if (reportFiles.length > 0) {
      ...
      try {
        ...
        console.log(`   Success Rate: ${((reportData.summary.passedTests / reportData.summary.totalTests) * 100).toFixed(1)}%`);
        console.log(`   Total Duration: ${(reportData.summary.totalDuration / 1000).toFixed(2)}s`);
        
        if (reportData.summary.performanceMetrics.memoryStats) {
          const memStats = reportData.summary.performanceMetrics.memoryStats;
          console.log(`   Memory Growth: ${(memStats.growth / 1024 / 1024).toFixed(1)}MB`);
        }
        ...
        console.warn(`⚠️  Failed to parse performance report: ${error.message}`);
      }
    }
  }
  ...
  if (typeof global.gc === 'function') {
    try {
      ...
      console.warn(`⚠️  Failed to run final garbage collection: ${error.message}`);
    }
  }
