...
export class ConversationRenderer extends EventEmitter {
  ...
  private renderMessage(message: ConversationMessage, index: number): string[] {
    ...
    const contentLines = this.wrapLongText(
      message.content,
  ...
  private updateViewport(): void {
    ...
    // Horizontal viewport
    this.viewportState.visibleStartColumn = Math.floor(
      horizontalScrollPosition,
    );
  ...
  setScrollPosition(options: {
    ...
    if (options.position !== undefined) {
      ...
      const delta =
        options.direction === "up" ? -options.amount : options.amount;
  ...
  setHorizontalScrollPosition(options: {
    ...
    if (options.position !== undefined) {
      ...
      const delta =
        options.direction === "left" ? -options.amount : options.amount;
      newPosition = currentPos + delta;
    } else {
      return; // Invalid options
    }
    ...
    const maxHorizontalScroll = Math.max(
      0,
      this.viewportState.totalWidth - this.viewportState.viewportWidth,
    );
    ...
    this.emit(
      ...
      this.viewportState.horizontalScrollPosition,
    );
  }
  ...
  getVisibleLines(): { lines: string[]; startIndex: number; endIndex: number } {
    ...
    if (this.config.showBoundaryIndicators && lines.length > 0) {
      ...
      const isAtBottom =
        this.viewportState.scrollPosition >=
        this.viewportState.totalHeight - this.viewportState.viewportHeight;
  ...
  getPerformanceMetrics(): {
    ...
    const totalLines = this.renderedLines.length;
    const visibleLines =
      this.viewportState.visibleEndLine -
      this.viewportState.visibleStartLine +
      1;
  ...
  updateConfig(newConfig: Partial<RenderConfig>): void {
    ...
    if (
      newConfig.enableWordWrap !== undefined ||
