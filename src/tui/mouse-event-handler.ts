...
export class MouseEventHandler extends EventEmitter {
  ...
  private parseStandardMouseEvent(
    button: number,
    x: number,
    y: number,
  ): MouseEvent | null {
  ...
  private parseSGRMouseEvent(sequence: string): MouseEvent | null {
    ...
    let eventType: MouseEventType = pressed
    ...
    if (button === 64 || button === 65) {
      ...
      eventButton =
        button === 64 ? MouseButton.WHEEL_UP : MouseButton.WHEEL_DOWN;

      // Convert vertical scroll to horizontal when shift is pressed
      if (modifiers.shift) {
        eventButton =
          button === 64 ? MouseButton.WHEEL_LEFT : MouseButton.WHEEL_RIGHT;
      }
    }
    ...
    if (button === 66 || button === 67) {
      eventType = MouseEventType.SCROLL;
      eventButton =
        button === 66 ? MouseButton.WHEEL_LEFT : MouseButton.WHEEL_RIGHT;
    }
  ...
  processEvent(event: MouseEvent): {
    ...
    try {
      ...
      if (
        ...
        if (
          timeDiff <= this.config.doubleClickDelay &&
  ...
  private createDebouncedScrollListener(
