...
} from "./components/StreamingMessage.js";
...
export function App() {
  ...
  // Workspace trust gating (Claude parity)
  const [trustPrompt, setTrustPrompt] = useState<boolean>(false);
  const [trustPath] = useState<string>(process.cwd());

  // Streaming message state
  const [streamingMessage, setStreamingMessage] =
    useState<StreamingConversationMessage | null>(null);
  ...
  const [keyboardState, setKeyboardState] = useState<KeyboardState>({
    ...
    mouseMode: parity ? false : true, // Disable in parity to reduce flicker
    pasteBuffer: "",
    pasteTimeout: null,
    pasteMode: false,
  });
  ...
  useEffect(() => {
    (async () => {
      // Trust check before anything else
      const trusted = await isWorkspaceTrusted(trustPath);
      if (!trusted) {
        setTrustPrompt(true);
      }
      ...
      if (context) {
        setLines((prev) => [
          ...prev,
          "",
          "✓ Loaded project context from PLATO.md",
        ]);
      }
    })();
  }, []);
  ...
  async function isWorkspaceTrusted(cwd: string): Promise<boolean> {
    try {
      const file = path.join(cwd, ".plato", "trust.json");
      const txt = await fs.readFile(file, "utf8");
      const data = JSON.parse(txt || "{}");
      return !!data?.trusted?.[cwd];
    } catch {
      return false;
    }
  }

  async function setWorkspaceTrusted(cwd: string): Promise<void> {
    const dir = path.join(cwd, ".plato");
    await fs.mkdir(dir, { recursive: true });
    const file = path.join(dir, "trust.json");
    let data: any = { trusted: {} };
    try {
      data = JSON.parse(await fs.readFile(file, "utf8"));
    } catch {}
    data.trusted = data.trusted || {};
    data.trusted[cwd] = true;
  ...
  useInput(
    isRawModeSupported
      ? (inputKey, key) => {
          // Intercept trust prompt first (Claude parity)
          if (trustPrompt) {
            if (key.return || inputKey === "1") {
              // Accept
              setWorkspaceTrusted(trustPath)
                .then(() => setTrustPrompt(false))
                .catch(() => setTrustPrompt(false));
            } else if (
              key.escape ||
              inputKey === "2" ||
              inputKey.toLowerCase() === "n"
            ) {
              // Exit app
              const { exit } = useApp();
              exit();
          ...
          if (currentState.historyMode) {
            ...
            if (key.upArrow || key.downArrow) {
              ...
              if (newIndex >= 0) {
                ...
                setLines((prev) =>
                  prev.concat(
                    `Selected: ${selectedMessage.content.substring(0, 100)}...`,
                  ),
                );
              }
              return;
            ...
            if (key.return && currentState.selectedHistoryIndex >= 0) {
              ...
              const selectedMessage =
                currentState.messageHistory[currentState.selectedHistoryIndex];
              setKeyboardState((prev) => ({
                ...prev,
                historyMode: false,
                selectedHistoryIndex: -1,
            ...
            setConversationMessages([
              {
                role: "system",
                content:
                  "Screen cleared. Previous conversation preserved in memory.",
                timestamp: Date.now(),
                metadata: {},
              },
            ]);
          ...
          if (
            ...
            (key.ctrl && inputKey.toLowerCase() === "h")
          ) {
            // Ctrl+H
            handleBackspace();
            return;
  ...
  const showMessageHistory = async () => {
    ...
    setLines((prev) =>
      prev.concat(
        ...
        ...userMessages.map(
          (msg, i) =>
            `  ${i + 1}. ${msg.content.substring(0, 80)}${msg.content.length > 80 ? "..." : ""}`,
        ),
      ),
    );
  };
  ...
  const handleBackgroundMode = async () => {
    ...
    setLines((prev) =>
      prev.concat(
        "🔄 Background mode enabled - commands will run in background",
      ),
    );
  };
  ...
  const handleImagePaste = async () => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`❌ Image paste failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleDeleteWord = () => {
    setKeyboardState((prev) => {
      ...
      const newInput =
        lastSpaceIndex === -1 ? "" : input.slice(0, lastSpaceIndex + 1);
  ...
  const handleSlashCommand = async (text: string) => {
    ...
    // In static/quiet mode on Windows Terminal, avoid verbose handlers that repaint frequently.
    const staticMode =
    ...
    const post = (msg: string) =>
      setConversationMessages((prev) => [
        ...prev,
        { role: "system" as const, content: msg, timestamp: Date.now() },
      ]);

    if (command === "/help") {
      // Claude-style help: header, monospaced alignment, hyphen separator
      const pad = (s: string, n: number) => (s + " ".repeat(n)).slice(0, n);
      const lines = SLASH_COMMANDS.map(
        (c) => `  ${pad(c.name, 12)} - ${c.summary}`,
      );
      const helpText = ["Available commands:", ...lines].join("\n");
      setConversationMessages((prev) => [
        ...prev,
        {
    ...
    if (command === "/status") {
      ...
      post(
        `status: provider=${cfg.provider?.active} model=${cfg.model?.active} account=${auth.user?.login ?? (auth.loggedIn ? "logged-in" : "logged-out")}`,
      );
      return;
    }
    ...
    if (staticMode) {
      // Condensed static-mode outputs for all other commands
      if (command === "/model") {
      ...
      post(`Executed ${command}${args ? " " + args : ""}`);
      return;
    }
    ...
    if (command === "/init") {
      ...
      try {
        ...
        setLines((prev) =>
          prev.concat(`❌ Failed to generate PLATO.md: ${e?.message || e}`),
        );
      }
      return;
    }
    ...
    if (command === "/bashes") {
      try {
        ...
        setLines((prev) =>
          prev.concat(`❌ Bash command failed: ${e?.message || e}`),
        );
      }
      return;
    }
    ...
    if (command === "/hooks") {
      try {
        ...
        setLines((prev) =>
          prev.concat(`❌ Hook management failed: ${e?.message || e}`),
        );
      }
      return;
    }
      ...
      try {
        ...
        if (issues.length === 0) {
          ...
          setLines((prev) =>
            prev.concat(
              "⚠️ Security issues:",
              ...issues.map((i: any) => `  [${i.severity}] ${i.message}`),
            ),
          );
        }
      } catch (e: any) {
        setLines((prev) =>
          prev.concat(`❌ Security review failed: ${e?.message || e}`),
        );
      }
      return;
    }
    ...
    if (command === "/vim") {
      try {
        ...
        setLines((prev) =>
          prev.concat(`❌ Failed to toggle vim mode: ${e?.message || e}`),
        );
      }
      return;
    }
    ...
    if (command === "/privacy-settings") {
      try {
        ...
        setLines((prev) =>
          prev.concat(`❌ Privacy settings error: ${e?.message || e}`),
        );
      }
      return;
    }
    ...
    if (command === "/release-notes") {
      try {
        ...
        setLines((prev) =>
          prev.concat(`❌ Failed to read release notes: ${e?.message || e}`),
        );
      }
      return;
    }
  ...
  const handleIdeCommand = async (editor?: string) => {
    try {
      ...
      if (editor && !supportedEditors.includes(editor.toLowerCase())) {
        setLines((prev) =>
          prev.concat(
            `⚠️ Unsupported editor '${editor}'. Supported: ${supportedEditors.join(", ")}`,
          ),
        );
        return;
      }
      ...
      if (editor) {
        setLines((prev) =>
          prev.concat(
            `🔌 Connecting to ${editor.charAt(0).toUpperCase() + editor.slice(1)}...`,
          ),
        );
        ...
        setLines((prev) =>
          prev.concat(
            `🔌 IDE connection established. Auto-detecting editor...`,
          ),
        );
        setLines((prev) =>
          prev.concat(
            `✅ IDE features enabled: file awareness, linter warnings, jump-to-definition`,
          ),
        );
      }
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ IDE connection failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleInstallGitlabAppCommand = async () => {
    try {
      ...
      try {
        ...
        setLines((prev) =>
          prev.concat(`✅ Opened browser to configure GitLab integrations`),
        );
        setLines((prev) =>
          prev.concat(
            `   Configure integrations to enable automatic MR reviews`,
          ),
        );
      } catch {
        ...
        setLines((prev) =>
          prev.concat(
            `   Configure GitLab integrations to enable automatic MR reviews`,
          ),
        );
      }
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ GitLab integration setup failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleStatuslineCommand = async (args: string) => {
    try {
      ...
      if (parts[0] === "on" || parts[0] === "off" || parts[0] === "toggle") {
        const enabled =
          parts[0] === "on" || (parts[0] === "toggle" && !cfg?.statusline);
        await setConfigValue("statusline", String(enabled));
        setCfg(await loadConfig());
        setLines((prev) =>
          prev.concat(`Statusline ${enabled ? "enabled" : "disabled"}`),
        );
      } else {
      ...
      setLines((prev) =>
        prev.concat(`❌ Statusline config failed: ${e?.message || e}`),
      );
    }
  };
    ...
    try {
      ...
      if (parts.length >= 3) {
        ...
        if (action === "allow" || action === "deny") {
          ...
          setLines((prev) =>
            prev.concat(`✅ Permission set: ${scope}.${tool} = ${action}`),
          );
        } else {
          setLines((prev) =>
            prev.concat(`❌ Invalid action. Use 'allow' or 'deny'`),
          );
        }
      } else {
        setLines((prev) =>
      ...
      setLines((prev) =>
        prev.concat(`❌ Permission management failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleAddDirCommand = async (path: string) => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`✅ Added directory to context: ${path.trim()}`),
      );
      // TODO: Implement actual context addition
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Failed to add directory: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleCostCommand = async (args: string) => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`❌ Cost calculation failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleDoctorCommand = async (args: string) => {
    try {
      ...
      try {
        ...
        setLines((prev) =>
          prev.concat(
            `${auth.loggedIn ? "✅" : "❌"} Authentication: ${auth.loggedIn ? "valid" : "missing"}`,
          ),
        );
      } catch {
      ...
      setLines((prev) =>
        prev.concat(`❌ Diagnostics failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleMcpCommand = async (args: string) => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`❌ MCP command failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleTodosCommand = async (args: string) => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`❌ TODO command failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleProxyCommand = async (args: string) => {
    try {
      ...
      if (subcommand === "start") {
        const port =
          parts.find((p) => p.startsWith("--port"))?.split("=")[1] || "11434";
        setLines((prev) =>
          prev.concat(
      ...
      setLines((prev) =>
        prev.concat(`❌ Proxy command failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleKeydebugCommand = async (args: string) => {
    try {
      setLines((prev) =>
        prev.concat("🔑 Key debug mode activated. Press any key..."),
      );
      // TODO: Implement actual key capture
      setLines((prev) =>
        prev.concat("ℹ️ Next keypress will show raw bytes for debugging"),
      );
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Key debug failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleApplyModeCommand = async (args: string) => {
    try {
      ...
      if (mode === "auto" || mode === "off") {
        ...
        if (mode === "auto") {
          setLines((prev) =>
            prev.concat("  Patches will be applied automatically"),
          );
        } else {
          setLines((prev) =>
            prev.concat("  Patches require manual /apply command"),
          );
        }
      } else {
      ...
      setLines((prev) =>
        prev.concat(`❌ Apply mode failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleTerminalSetupCommand = async () => {
    try {
      ...
      setLines((prev) =>
        prev.concat(
          `   Detected: ${termProgram}, Shell: ${shell.split("/").pop()}`,
        ),
      );
      ...
      switch (termProgram) {
        case "vscode":
          ...
          setLines((prev) =>
            prev.concat(
              `   2. Search for 'terminal.integrated.sendKeybindingsToShell'`,
            ),
          );
          setLines((prev) =>
            prev.concat(`   3. Set to false to fix Shift+Enter`),
          );
          break;
        case "iTerm.app":
          setLines((prev) => prev.concat(`📝 iTerm2 Instructions:`));
          setLines((prev) =>
            prev.concat(`   1. Go to Preferences > Profiles > Keys`),
          );
          setLines((prev) =>
            prev.concat(
              `   2. Add key mapping: Shift+Enter → 'Send text: \\n'`,
            ),
          );
          break;
        case "Terminal":
          ...
          setLines((prev) =>
            prev.concat(`   1. Terminal > Preferences > Profiles > Keyboard`),
          );
          setLines((prev) =>
            prev.concat(`   2. Check 'Use option as meta key'`),
          );
          break;
        default:
          setLines((prev) => prev.concat(`📝 Generic Terminal Instructions:`));
          setLines((prev) =>
            prev.concat(`   1. Check terminal key binding settings`),
          );
          setLines((prev) =>
            prev.concat(`   2. Ensure Shift+Enter sends newline character`),
          );
      }
      ...
      setLines((prev) =>
        prev.concat(`   - Check shell configuration (.bashrc, .zshrc)`),
      );
      setLines((prev) =>
        prev.concat(
          `   - Test with: printf "Line 1\\nLine 2" (should show two lines)`,
        ),
      );
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Terminal setup failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleCompactCommand = async (instructions?: string) => {
    try {
      ...
      if (instructions) {
        setLines((prev) =>
          prev.concat(`📋 Focus instructions: "${instructions}"`),
        );
        setLines((prev) =>
          prev.concat(
            `✅ Smart compaction applied with focus on: ${instructions}`,
          ),
        );
      } else {
        setLines((prev) =>
          prev.concat(`✅ Compacted conversation using intelligent strategy`),
        );
      }

      setLines((prev) =>
        prev.concat(
          `📊 Reduced from ${result.originalLength} messages to ${result.newLength} messages`,
        ),
      );
      ...
      setLines((prev) =>
        prev.concat(`❌ Compaction failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleBugCommand = async (description?: string) => {
    try {
      ...
      try {
        ...
        if (description) {
          setLines((prev) =>
            prev.concat(`💡 Please include this description: "${description}"`),
          );
        }
      } catch {
        setLines((prev) => prev.concat(`📋 Please visit: ${url}`));
        setLines((prev) =>
          prev.concat(`   Report bugs and feature requests for Plato`),
        );
      }
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Bug report failed: ${e?.message || e}`),
      );
    }
  };
      ...
      switch (action) {
        ...
        case "show": {
          ...
          if (memories.length === 0) {
            ...
            setLines((prev) =>
              prev.concat("📝 Recent memories:", ...memories.slice(-20)),
            );
          }
          break;
        }
        ...
        case "plato": {
          ...
          if (context) {
            setLines((prev) =>
              prev.concat("📄 PLATO.md content:", ...context.split("\n")),
            );
          } else {
            setLines((prev) =>
              prev.concat(
                "📝 No PLATO.md found. Create one with project context.",
              ),
            );
          }
          break;
        ...
        case "update-context": {
          if (rest) {
            ...
            setLines((prev) =>
              prev.concat("📝 Use: /memory update-context <content>"),
            );
          }
          break;
        }
        ...
        default:
          setLines((prev) =>
            prev.concat(
              `❓ Unknown memory command: ${action}. Use '/memory help' for options.`,
            ),
          );
      }
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Memory command failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleOutputStyleCommand = async (args?: string) => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`❌ Style command failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleCreateStyleCommand = async () => {
    try {
      ...
      setLines((prev) =>
        prev.concat(`❌ Style creation failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleModelCommand = async (args?: string) => {
    try {
      ...
      if (!args || args.trim() === "") {
        // Show available models and current selection in conversation feed (single system message)
        ...
        setConversationMessages((prev) => [
          ...prev,
          { role: "system" as const, content: body, timestamp: Date.now() },
        ]);
        return;
      }
      ...
      if (!availableModels.includes(targetModel)) {
        setConversationMessages((prev) => [
          ...
          {
            role: "system" as const,
            content: `❌ Model '${targetModel}' not available.\nUse /model to see available options.`,
            timestamp: Date.now(),
          },
        ]);
        return;
      ...
      setConversationMessages((prev) => [
        ...
        {
          role: "system" as const,
          content: `✅ Switched to model: ${targetModel}`,
          timestamp: Date.now(),
        },
      ]);
    } catch (e: any) {
      setConversationMessages((prev) => [
        ...
        {
          role: "system" as const,
          content: `❌ Model command failed: ${e?.message || e}`,
          timestamp: Date.now(),
        },
      ]);
    }
  ...
  const handleMouseCommand = async (args?: string) => {
    try {
      ...
      if (command === "on" || command === "enable") {
        ...
        setLines((prev) =>
          prev.concat(
            "🖱️  Mouse mode enabled - copy/paste should work better now",
          ),
        );
        ...
        setLines((prev) =>
          prev.concat(
            "🖱️  Mouse mode disabled - full keyboard handling restored",
          ),
        );
        ...
        setLines((prev) =>
          prev.concat(`🖱️  Mouse mode ${newMode ? "enabled" : "disabled"}`),
        );
      } else {
        setLines((prev) =>
          prev.concat(
            `❌ Unknown mouse command: ${command}. Use 'on', 'off', or 'toggle'.`,
          ),
        );
      }
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Mouse command failed: ${e?.message || e}`),
      );
    }
  };
  ...
  const handlePasteCommand = async (args?: string) => {
    try {
      ...
      if (isNaN(seconds) || seconds < 1 || seconds > 60) {
        setLines((prev) =>
          prev.concat("❌ Invalid duration. Use 1-60 seconds."),
        );
        return;
      }
      ...
      setTimeout(() => {
        ...
        setLines((prev) =>
          prev.concat("📋 Paste mode disabled - keyboard input restored"),
        );
      }, seconds * 1000);
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Paste command failed: ${e?.message || e}`),
      );
    }
  };
    ...
    try {
      ...
      if (result.success) {
        switch (result.action) {
          ...
          case "suggest":
            setLines((prev) =>
              prev.concat(
                ...
                result.data.suggestions.length > 5
                  ? `   ... and ${result.data.suggestions.length - 5} more`
                  : "",
                "",
              ),
            );
          ...
          case "add-related":
            setLines((prev) =>
              prev.concat(
                ...
                ...result.data.addedFiles
                  .slice(0, 3)
                  .map((f: string) => `   + ${f}`),
                result.data.addedFiles.length > 3
                  ? `   + ${result.data.addedFiles.length - 3} more...`
                  : "",
          ...
          case "optimize":
            setLines((prev) =>
              prev.concat(
                ...
                ...result.data.optimizations
                  .slice(0, 3)
                  .map((opt: string) => `   • ${opt}`),
                result.data.optimizations.length > 3
                  ? `   • ${result.data.optimizations.length - 3} more suggestions...`
                  : "",
                "",
              ),
            );
            ...
            setLines((prev) =>
              prev.concat(
                ...
                result.data.results.length > 5
                  ? `   ... ${result.data.totalMatches - 5} more results`
                  : "",
                "",
              ),
            );
          ...
          default:
            setLines((prev) =>
              prev.concat(`✅ Context command completed: ${result.action}`),
            );
        }
      } else {
        setLines((prev) =>
          prev.concat(`❌ Context command failed: ${result.error}`),
        );
      }
    } catch (e: any) {
      setLines((prev) =>
        prev.concat(`❌ Context command error: ${e?.message || e}`),
      );
    }
  };
  ...
  const handleRegularMessage = async (text: string) => {
    ...
    try {
      await orchestrator.respondStream(
        ...
        (evt) => {
          if (evt.type === "info")
            setLines((prev) => prev.concat(evt.message || "info"));
          if (evt.type === "tool-start")
            setLines((prev) => prev.concat(`tool: ${evt.message}`));
          if (evt.type === "tool-end")
            setLines((prev) => prev.concat("tool: done"));
        },
      );
    } catch (e: any) {
      ...
      const currentStreaming =
        streamingManagerRef.current?.getCurrentStreamingMessage();
      if (currentStreaming && currentStreaming.content) {
        ...
        setConversationMessages((prev) => {
          ...
          if (
            last &&
            last.role === "assistant" &&
            last.content?.trim() === finalMessage.content.trim()
          ) {
            // Avoid duplicate assistant message when stream already rendered final content
            return prev;
          }
  ...
  const handleRightClick = (x: number, y: number, selectedText?: string) => {
    setStatus(
      `Right-click at (${x}, ${y})${selectedText ? ` with selection` : ""}`,
    );
  ...
  const statusline = React.useMemo(() => {
    const fmt =
      cfg?.statusline?.format ||
      "plato | {provider} | {model} | {tokens} {branch}";
    const m = metrics;
    ...
    const map: Record<string, string> = {
      ...
      cwd: process.cwd(),
      mode: keyboardState.transcriptMode
        ? "transcript"
        : keyboardState.backgroundMode
          ? "background"
          : "chat",
  ...
  if (!parity) {
    ...
    if (keyboardState.mouseMode && !keyboardState.pasteMode)
      modeIndicators.push("🖱️  MOUSE");
  }

  if (trustPrompt) {
    // Minimal trust modal approximating Claude's copy
    const p = trustPath;
    return (
      <Box flexDirection="column">
        ...
        <Text>{`│                                                                              │`}</Text>
        <Text>{`│ ${p.padEnd(76).slice(0, 76)} │`}</Text>
        <Text>{`│                                                                              │`}</Text>
        <Text>{`│ Claude Code may read, write, or execute files contained in this directory.   │`}</Text>
        <Text>{`│ This can pose security risks, so only use files from trusted sources.        │`}</Text>
        <Text>{`│                                                                              │`}</Text>
        <Text>{`│ Learn more ( https://docs.anthropic.com/s/claude-code-security )             │`}</Text>
        <Text>{`│                                                                              │`}</Text>
        <Text>{`│ ❯ 1. Yes, proceed                                                            │`}</Text>
        <Text>{`│   2. No, exit                                                                │`}</Text>
        <Text>{`│                                                                              │`}</Text>
        <Text>
          {`╰──────────────────────────────────────────────────────────────────────────────╯`}
        </Text>
        <Text>{`   Enter to confirm · Esc to exit`}</Text>
      </Box>
    );
  }
  ...
  return (
    ...
    >
      ...
      />
      {modeIndicators.length > 0 && conversationMessages.length > 0 && (
        <Box>
          <StatusLine
...
export async function runTui() {
  // Comprehensive environment and capability checks before attempting Ink render
  const env = getTerminalEnvironment();
  const isRawModeSupported = process.stdin.setRawMode !== undefined;
  const hasTTY = process.stdin.isTTY && process.stdout.isTTY;

  // Check for compatibility mode flag or force TUI attempt
  const forceTUI = process.env.PLATO_FORCE_TUI === "true";

  // Fail fast for any unsupported environment to prevent Ink initialization attempts
  // But allow forcing TUI for testing/development
  if (
    !forceTUI &&
    (!isRawModeSupported || !hasTTY || env.isDocker || env.isCI)
  ) {
    throw new Error("Terminal environment does not support TUI interface");
  }

  // Special handling for WSL - allow attempt but with warnings
  if (!forceTUI && env.isWSL && (!isRawModeSupported || !hasTTY)) {
    throw new Error("Terminal environment does not support TUI interface");
  }

  try {
    // In quiet mode, reduce Ink frequency by ensuring minimal logging to stdout
