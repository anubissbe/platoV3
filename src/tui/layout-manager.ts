...
export class LayoutManager {
  ...
  expandPanel(id: string): void {
    ...
    if (hasPreCollapseWidths) {
      ...
      const previousWidth =
        this.preCollapseWidths.get(id) || panel.config.defaultWidth;
      ...
      const previousWidth =
        this.previousWidths.get(id) || panel.config.defaultWidth;
      panel.state.width = previousWidth;
    }
  ...
  resizePanel(id: string, newWidth: number): void {
    ...
    if (otherPanels.length > 0 && Math.abs(widthChange) > 0.1) {
      ...
      const totalOtherWidth = otherPanels.reduce(
        (sum, p) => sum + p.state.width,
        0,
      );
      ...
      if (totalOtherWidth > 0) {
        ...
        const currentTotal = visiblePanels.reduce(
          (sum, p) => sum + p.state.width,
          0,
        );
  ...
  handleTerminalResize(columns: number, rows: number): void {
    ...
    if (
      columns < this.MIN_TERMINAL_WIDTH_FOR_MULTI &&
  ...
  serialize(): LayoutConfig {
    const panels = Array.from(this.panels.values()).map((panel) => {
      ...
      if (
        this.layoutMode === "single" &&
        this.savedLayoutWidths.has(panel.id)
      ) {
        serializedPanel.state.width =
          this.savedLayoutWidths.get(panel.id) || panel.state.width;
      }
  ...
  protected getVisiblePanels(): PanelInfo[] {
    return Array.from(this.panels.values()).filter(
      (p) => p.state.visible && !p.state.collapsed,
    );
  }
    ...
    if (Math.abs(totalWidth - 100) > 0.1) {
      ...
      if (visiblePanels.length === 1) {
        ...
        if (focused && other) {
          ...
          focusedWidth = Math.max(
            focusedMin,
            Math.min(focusedMax, focusedWidth),
          );
          otherWidth = 100 - focusedWidth;
