...
import { BREAKPOINTS, getBreakpoint, ResponsiveMode, getResponsiveMode } from './components/ResponsiveContainer.js';
...
export class ResponsiveLayoutManager extends LayoutManager {
  ...
  private applyFlexibleSizing(columns: number, rows: number): void {
    ...
    const availableWidth = columns - (borderWidth * visiblePanels.length) - padding;
  ...
  private updateMobileOptimizations(): void {
    ...
    if (this.responsiveConfig.enableMobileSupport) {
      this.eventEmitter.emit('layout:mobile:optimizations', this.mobileOptimizations);
    }
  }
  ...
  private updatePerformanceMetrics(resizeTime: number): void {
    ...
    const total = this.resizePerformanceMetrics.averageResizeTime * 
                  (this.resizePerformanceMetrics.totalResizes - 1) + resizeTime;
