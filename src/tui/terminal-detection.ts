...
export class TerminalCapabilities {
  ...
  private detectTerminalApp(): { name: string; version?: string } {
    ...
    // PowerShell
    if (process.platform === 'win32' && process.env.PSModulePath && !termProgram) {
   ...
   */
  private detectColorSupport(): 'none' | 'basic' | '16color' | '256color' | 'truecolor' {
   ...
   * Detect WSL environment
   */
  private detectWSL(): { isWSL?: boolean; wslVersion?: number; wslDistro?: string } {
    ...
    if (release.includes('microsoft-standard-WSL2') || release.includes('WSL2')) {
      wslVersion = 2;
    }
