...
export class KeyboardShortcutManager {
  ...
  private registerDefaultShortcuts() {
    ...
    this.register({
      ...
      binding: [{ key: "F2" }, { key: "p", modifiers: { ctrl: true } }],
    ...
    this.register({
      ...
      binding: [{ key: "F3" }, { key: "f", modifiers: { ctrl: true } }],
  ...
  register(shortcut: ShortcutDefinition): void {
    ...
    for (const key of bindingKeys) {
      ...
      if (existing && existing.id !== shortcut.id) {
        console.warn(
          `Keyboard shortcut conflict: ${key} is already bound to ${existing.name}`,
        );
      }
    }
  ...
  private async toggleDetailedCostAnalytics(): Promise<void> {
    try {
      const { toggleDetailedCostAnalytics } = await import(
        "./status-config.js"
      );
      const enabled = await toggleDetailedCostAnalytics();
      console.log(
        `Detailed cost analytics ${enabled ? "enabled" : "disabled"}`,
      );
    } catch (error) {
