...
export function VirtualScrollList<T extends VirtualScrollItem>({
  ...
  const getItemMetrics = useCallback(
    (index: number): { offset: number; height: number } => {
      ...
      const cachedHeight =
        heightCache.current.get(item.id) || item.height || estimatedItemHeight;
      ...
      for (let i = 0; i < index; i++) {
        ...
        if (prevItem) {
          offset +=
            heightCache.current.get(prevItem.id) ||
            prevItem.height ||
            estimatedItemHeight;
        }
      }
  ...
  return (
    ...
    >
      ...
      {scrollMetrics.visibleEndIndex < items.length - 1 && (
        <Box
          height={Math.floor(
            (scrollMetrics.totalHeight -
              scrollMetrics.offsetY -
              viewportHeight) /
              estimatedItemHeight,
          )}
        />
