  ...
  ({
    ...
    return (
      ...
      >
        ...
        {/* Message content */}
        <Box paddingLeft={2}>
          <Text wrap="wrap">{processedContent}</Text>
        </Box>
...
export const MemoizedHeader = memo<MemoizedHeaderProps>(
  ({ title, subtitle, status = "disconnected", tokens, maxTokens }) => {
...
export const MemoizedInputArea = memo<MemoizedInputAreaProps>(
  ({
    ...
    return (
      <Box
        flexDirection="column"
        borderStyle={process.env.PLATO_QUIET_TUI === "1" ? undefined : "single"}
        paddingX={1}
        height={isMultiline ? height + 2 : 3}
      >
        {displayLines.length === 0 && <Text color="gray">{placeholder}</Text>}
