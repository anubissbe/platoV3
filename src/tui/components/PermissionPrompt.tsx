...
export const PermissionPrompt: React.FC<PermissionPromptProps> = ({
  ...
  const [selectedOption, setSelectedOption] = useState<'allow' | 'deny' | 'skip'>('allow');
  ...
  return (
    <Box flexDirection="column" borderStyle="double" borderColor="yellow" padding={1}>
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        {query.operation && (
          <Box>
            <Text color="cyan" bold>Operation: </Text>
        ...
        {query.command && (
          <Box>
            ...
            <Text color="white">{query.command.substring(0, 60)}{query.command.length > 60 ? '...' : ''}</Text>
          </Box>
        )}
      </Box>
      ...
      {rule && (
        <Box flexDirection="column" marginBottom={1}>
          <Text color="magenta" bold>Triggered Rule:</Text>
          <Box marginLeft={2}>
      ...
      {profile && (
        <Box marginBottom={1}>
          <Text color="cyan" bold>Active Profile: </Text>
          <Text color="white">{profile.name}</Text>
          <Box marginLeft={2}>
            <Text color="gray" dimColor>({profile.description})</Text>
          </Box>
        </Box>
      )}
      ...
      {/* Suggested Action */}
      <Box marginBottom={1}>
        <Text color="green" bold>Suggested: </Text>
        <Text color={suggestedAction === 'allow' ? 'green' : suggestedAction === 'deny' ? 'red' : 'yellow'}>
      ...
      {/* Options */}
      <Box flexDirection="column" marginBottom={1}>
        <Text color="white" bold>Choose action:</Text>
      ...
      {showDetails && (
        <Box flexDirection="column" borderStyle="single" borderColor="gray" padding={1} marginBottom={1}>
          <Text color="gray" bold>Operation Context:</Text>
          <Box marginLeft={2}>
      ...
      <Box flexDirection="column" borderTop borderColor="gray" paddingTop={1}>
        <Text color="gray" dimColor>
          Controls: ↑↓ navigate • Enter confirm • r remember • d details {onElevate && '• e elevate'} • Esc skip
        </Text>
      </Box>
    </Box>
  );
