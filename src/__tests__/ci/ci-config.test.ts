...
describe("CI Configuration Validation", () => {
  ...
  describe("GitHub Actions Workflow", () => {
    ...
    test("should use matrix strategy for Node.js versions", () => {
      ...
      const nodeVersions =
        testJob.strategy?.matrix?.node ||
        testJob.strategy?.matrix?.["node-version"];
    ...
    test("should have proper job steps", () => {
      ...
      const hasTest = steps.some(
        (step) =>
          step.run?.includes("npm test") || step.run?.includes("npm run test"),
      );
    ...
    test("should run tests with coverage", () => {
      ...
      const testJob = workflowContent.jobs.test;
      const hasTestWithCoverage = testJob.steps.some(
        (step) =>
    ...
    test("should include linting step", () => {
      ...
      const testJob = workflowContent.jobs.test;
      const hasLint = testJob.steps.some((step) =>
    ...
    test("should include type checking step", () => {
      ...
      const testJob = workflowContent.jobs.test;
      const hasTypecheck = testJob.steps.some(
        (step) =>
          step.run?.includes("npm run typecheck") || step.run?.includes("tsc"),
      );
    ...
    test("should have coverage reporting step", () => {
      ...
      const testJob = workflowContent.jobs.test;
      const hasCoverageUpload = testJob.steps.some(
        (step) =>
  ...
  describe("README Coverage Badges", () => {
    ...
    test("should include coverage badge", () => {
      ...
      const hasCoverageBadge =
        readmeContent.includes("![Coverage]") ||
        readmeContent.includes("![coverage]");
      const hasGitLabCoverageBadge =
        readmeContent.includes("/badges/") &&
        readmeContent.includes("coverage");
      const hasGenericCoverageBadge =
        readmeContent.includes("coverage-") && readmeContent.includes("%");

      expect(
        hasCodecovBadge ||
          hasCoverallsBadge ||
          hasCoverageBadge ||
          hasGitLabCoverageBadge ||
          hasGenericCoverageBadge,
      ).toBe(true);
    });

    test("should include build status badge", () => {
      if (!readmeExists) return;
      const hasGitHubBuildBadge =
        readmeContent.includes("github.com/") &&
        (readmeContent.includes("/workflows/") ||
          readmeContent.includes("/actions/"));
      const hasGitLabPipelineBadge =
        readmeContent.includes("/badges/") &&
        (readmeContent.includes("pipeline") || readmeContent.includes("build"));
      const hasGenericBuildBadge =
        readmeContent.includes("![Pipeline") ||
        readmeContent.includes("![Build");

      expect(
        hasGitHubBuildBadge || hasGitLabPipelineBadge || hasGenericBuildBadge,
      ).toBe(true);
    });
  });
