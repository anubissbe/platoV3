...
describe("Permissions System Behavior Analysis", () => {
  ...
  test("should analyze project config behavior", async () => {
    const projectConfig = {
      permissions: {
        defaults: { mcp: "deny" },
        rules: [{ match: { tool: "mcp", path: "/safe/*" }, action: "allow" }],
      },
    };
  ...
  test("should analyze global + project merging behavior", async () => {
    const globalConfig = {
      permissions: {
        ...
        rules: [{ match: { tool: "mcp", path: "/global/*" }, action: "allow" }],
      },
    };
    ...
    const result4 = await checkPermission({
      tool: "mcp",
      path: "/project/file",
    });

    console.log("Merged config results:", {
      result1,
      result2,
      result3,
      result4,
  ...
  test("should analyze savePermissions and project API behavior", async () => {
    ...
    console.log(
      "After addPermissionRule:",
      JSON.stringify(permissions2, null, 2),
    );
  ...
  test("should analyze rule matching behavior", async () => {
    ...
    results.push([
      "path /allow/file",
      await checkPermission({ tool: "test_tool", path: "/allow/file" }),
    ]);
    results.push([
      "path /deny/file",
      await checkPermission({ tool: "test_tool", path: "/deny/file" }),
    ]);
    results.push([
      "path /other/file",
      await checkPermission({ tool: "test_tool", path: "/other/file" }),
    ]);

    // Test command matching
    results.push([
      "command safe_op",
      await checkPermission({ tool: "test_tool", command: "safe_operation" }),
    ]);
    results.push([
      "command unsafe_op",
      await checkPermission({ tool: "test_tool", command: "unsafe_operation" }),
    ]);

    // Test no path/command
    results.push([
      "no path/command",
      await checkPermission({ tool: "test_tool" }),
    ]);
