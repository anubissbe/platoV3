...
describe('MCP Integration Tests', () => {
  ...
  describe('Server Management', () => {
    ...
    it('should prevent duplicate server attachment', async () => {
      ...
      await expect(attachServer('test-server', serverUrl))
        .rejects
        .toThrow('mcp server exists: test-server');
    });
  ...
  describe('Tool Discovery', () => {
    ...
    it('should list tools from all servers when no server specified', async () => {
      ...
      expect(toolsList.map(t => t.server).sort()).toEqual(['server1', 'server2']);
    });
    ...
    it('should handle server with no tools gracefully', async () => {
      ...
      const emptyServer = http.createServer((req, res) => {
        if (req.method === 'GET' && (req.url === '/tools' || req.url === '/.well-known/mcp/tools')) {
  ...
  describe('Tool Execution', () => {
    ...
    it('should execute echo tool', async () => {
      const result = await callTool('test-server', 'echo', { text: 'hello world' });
    ...
    it('should handle tool execution errors', async () => {
      await expect(callTool('test-server', 'nonexistent', {}))
        .rejects
        .toThrow();
    });

    it('should handle nonexistent server', async () => {
      await expect(callTool('nonexistent-server', 'echo', { text: 'test' }))
        .rejects
    ...
    it('should not retry on non-retryable errors', async () => {
      const unauthorizedServer = http.createServer((req, res) => {
        if (req.method === 'GET' && (req.url === '/tools' || req.url === '/.well-known/mcp/tools')) {
          ...
          res.end(JSON.stringify({ 
            tools: [
              { name: 'unauthorized', description: 'Unauthorized tool', input_schema: {} }
            ] 
          }));
          return;
        }
      ...
      const unauthorizedPort = (unauthorizedServer.address() as AddressInfo).port;
      const unauthorizedUrl = `http://localhost:${unauthorizedPort}`;
      
      try {
        await attachServer('unauthorized-server', unauthorizedUrl);
        
        await expect(callTool('unauthorized-server', 'unauthorized', {}))
          .rejects
          .toThrow('Tool call failed: 401 Unauthorized');
      } finally {
