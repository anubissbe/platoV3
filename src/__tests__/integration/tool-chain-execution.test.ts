...
describe("Tool Chain Execution Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "plato-tool-chain-test-"),
    );
...
`,
    ...
    await fs.writeFile(path.join(".plato", "mcp-servers.json"), "[]");
  ...
  describe("Tool Chain Coordination", () => {
    test("should execute tool chain with proper coordination", async () => {
      ...
      const result1 = await callTool("tool-test-server", "prepare_data", {
        input: "test data",
      });
      ...
      const result2 = await callTool("tool-test-server", "process_data", {
        data: result1.data,
      });
      ...
      const result3 = await callTool("tool-test-server", "finalize_data", {
        processed: result2.processed_data,
      });
    ...
    test("should handle tool chain failures gracefully", async () => {
      ...
      await expect(
        callTool("tool-test-server", "failing_tool", { input: "fail" }),
      ).rejects.toThrow();

      // Verify system can recover and continue
      const result = await callTool("tool-test-server", "prepare_data", {
        input: "recovery test",
      });
  ...
  describe("Tool Result Serialization", () => {
    ...
    test("should handle large data serialization", async () => {
      const largeData = {
        large_array: new Array(100)
          .fill(0)
          .map((_, i) => ({ id: i, value: `item-${i}` })), // Reduced size for faster test
      };
      ...
      expect(result.echoed.large_array[99]).toEqual({
        id: 99,
        value: "item-99",
      });
    });
  ...
  describe("Tool Error Handling", () => {
    test("should propagate tool errors with proper context", async () => {
      try {
        await callTool("tool-test-server", "failing_tool", {
          input: "error test",
        });
  ...
  describe("Tool Lifecycle Management", () => {
    test("should properly manage tool lifecycle", async () => {
      ...
      const result1 = await callTool("tool-test-server", "initialize_tool", {
        config: "test",
      });
  ...
  describe("Orchestrator Integration", () => {
    test("should integrate with orchestrator for tool permission checks", async () => {
      ...
      const permission = await orchestrator.checkToolPermission("test_tool", {
        input: "test",
      });
        ...
        try {
          ...
          switch (toolName) {
            case "prepare_data":
              res.writeHead(200, { "Content-Type": "application/json" });
              res.end(
                JSON.stringify({ ok: true, data: input.input, prepared: true }),
              );
              return;
