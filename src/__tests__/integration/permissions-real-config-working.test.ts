...
describe("Permissions System Real Config Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    ((global as any).mockTempDirs =
      (global as any).mockTempDirs || new Set()).add(tempProjectDir);
    ((global as any).mockTempDirs =
      (global as any).mockTempDirs || new Set()).add(tempHomeDir);
  ...
  describe("Config Inheritance and Override Behavior", () => {
    test("should properly merge global and project configurations", async () => {
      ...
      const projectRule = permissions.rules?.find(
        (r) => r.match.path === "/project/sensitive/*",
      );
      const globalRule = permissions.rules?.find(
        (r) => r.match.path === "/global/restricted/*",
      );
