...
describe("Workflow Integration", () => {
  ...
  beforeEach(() => {
    ...
    mockExecuteMouseCommand.mockResolvedValue({
      success: true,
    ...
    mouseGuidance = new MouseUserGuidance(mockDefaultSettings, {
      supported: true,
      rightClick: false,
      scrollWheel: true,
      dragAndDrop: true,
  ...
  describe("Mouse-Keyboard Workflow Integration", () => {
    it("should seamlessly switch between mouse and keyboard input", () => {
      ...
      const suggestions = slashMouseIntegration.updateSuggestions("/mou", {
        x: 10,
        y: 5,
      });
    ...
    it("should provide keyboard alternatives in mouse guidance", () => {
      ...
      expect(guidance).toContain(
        "Right-click not available (use keyboard shortcuts)",
      );
  ...
  describe("Slash Command Integration", () => {
    it("should execute mouse commands via slash command interface", async () => {
      const result = await slashMouseIntegration.executeCommand("/mouse", [
        "on",
      ]);
    ...
    it("should provide contextual command suggestions based on mouse position", () => {
      ...
      const inputAreaSuggestions = slashMouseIntegration.getContextMenu({
        x: 10,
        y: 25,
      });
      expect(inputAreaSuggestions.some((s) => s.command === "/paste")).toBe(
        true,
      );

      const outputAreaSuggestions = slashMouseIntegration.getContextMenu({
        x: 10,
        y: 15,
      });
      expect(outputAreaSuggestions.some((s) => s.command === "/export")).toBe(
        true,
      );
    });
  ...
  describe("Session Persistence Integration", () => {
    ...
    it("should provide learned recommendations for settings", () => {
      ...
      const learnedSettings =
        preferencesManager.learnFromBehavior(behaviorData);
  ...
  describe("Performance and Accessibility Integration", () => {
    it("should adapt mouse sensitivity based on performance metrics", () => {
      ...
      const performanceWarning = guidanceMessages.find(
        (m) => m.type === "performance_warning",
      );
    ...
    it("should provide accessibility recommendations based on settings", () => {
      ...
      const accessibilityMessage = guidance.find(
        (m) => m.type === "accessibility",
      );
  ...
  describe("Integration Testing Scenarios", () => {
    it("should complete full mouse-keyboard workflow cycle", async () => {
      ...
      const enableResult = await slashMouseIntegration.executeCommand(
        "/mouse",
        ["on"],
      );
      ...
      const suggestions = slashMouseIntegration.updateSuggestions("/exp", {
        x: 10,
        y: 25,
      });
      expect(suggestions.length).toBeGreaterThan(0);
