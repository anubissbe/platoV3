...
describe("Horizontal Scrolling", () => {
  ...
  beforeEach(() => {
    ...
    conversationRenderer.addMessage({
      role: "assistant",
      content:
        "This is a very long line that extends beyond the normal terminal width and requires horizontal scrolling to view completely. It contains lots of text that would normally wrap but we have disabled word wrapping to test horizontal scrolling functionality.",
      timestamp: Date.now(),
    });
...
\`\`\``,
  ...
  describe("Wide Content Handling", () => {
    ...
    test("should scroll to end of wide content", () => {
      ...
      const maxScroll = Math.max(
        0,
        viewport.totalWidth - viewport.viewportWidth,
      );
    ...
    test("should scroll to start of content", () => {
      ...
      conversationRenderer.scrollToEnd();
      expect(
        conversationRenderer.getHorizontalScrollPosition(),
      ).toBeGreaterThan(0);
  ...
  describe("Horizontal Viewport Management", () => {
    ...
    test("should handle terminal width changes", () => {
      ...
      const narrowViewport =
        conversationRenderer["viewportState"].viewportWidth;
  ...
  describe("Combined Scrolling", () => {
    test("should handle both vertical and horizontal scrolling independently", () => {
      ...
      const initialVerticalPos =
        conversationRenderer.getCurrentScrollPosition();
      ...
      expect(conversationRenderer.getHorizontalScrollPosition()).toBe(
        horizontalPos,
      );
    });
