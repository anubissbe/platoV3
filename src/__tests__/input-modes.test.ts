...
jest.mock("../runtime/orchestrator", () => ({
  orchestrator: {
    ...
    pasteImageFromClipboard: jest.fn(() =>
      Promise.resolve({ success: false, message: "Not supported" }),
    ),
...
describe.skip("Input Modes Tests", () => {
  ...
  describe("Mode-Specific Input Handling", () => {
    test("should handle normal mode input", () => {
      ...
      expect(mockInputModeManager.modes.normal.onInput).toHaveBeenCalledWith(
        inputEvent,
      );
    });
    ...
    test("should handle command mode input differently", () => {
      ...
      expect(mockInputModeManager.modes.command.onInput).toHaveBeenCalledWith(
        inputEvent,
      );
    });
    ...
    test("should handle search mode input", () => {
      ...
      expect(mockInputModeManager.modes.search.onInput).toHaveBeenCalledWith(
        inputEvent,
      );
    });
  ...
  describe("Search Mode Functionality", () => {
    test("should search through conversation history", () => {
      const conversationHistory = [
        ...
        {
          role: "assistant",
          content: "Use OAuth 2.0 for secure authentication",
        },
