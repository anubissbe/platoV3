...
describe("Long AI Response Scrolling", () => {
  ...
  describe("Large Response Handling", () => {
    test("should efficiently handle very long AI responses", () => {
      ...
      const { lines, startIndex, endIndex } =
        conversationRenderer.getVisibleLines();
...
This code demonstrates the concept.`;
      ...
      const codeLine = lines.find((line) =>
        line.includes("reallyLongVariableName"),
      );
  ...
  describe("Memory Efficiency", () => {
    ...
    test("should handle terminal width changes dynamically", () => {
      const message = {
        role: "assistant" as const,
        content:
          "This is a very long line that will wrap differently based on terminal width settings and should adapt properly to different screen sizes.",
        timestamp: Date.now(),
      };
...
And regular paragraphs that should be wrapped properly according to terminal width.`;
      ...
      const codeLines = lines.filter((line) =>
        line.includes("longVariableName"),
      );
      const paragraphLines = lines.filter(
        (line) =>
          line.includes("regular paragraphs") ||
          line.includes("wrapped properly"),
      );
