...
import { PerformanceMonitor, MemoryManager } from '../../tui/PerformanceMonitor';
...
interface ErrorScenario {
  ...
  delay: number; // ms
  recoverable: boolean;
  errorType: 'network' | 'memory' | 'disk' | 'timeout' | 'permission' | 'resource';
}
...
class ReliabilityTester {
  ...
  async runStabilityTest(
    ...
    const { duration, errorScenarios = [], expectedErrorRate = 0.05, isolatedTempDir = false } = config;
  ...
  private async createIsolatedTempDir(): Promise<string> {
    ...
    const testTempDir = path.join(baseTempDir, `plato-reliability-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  private shouldInjectError(scenarios: ErrorScenario[]): { inject: boolean; scenario?: ErrorScenario } {
  ...
  private calculateStabilityScore(params: {
    ...
    const { errorRate, expectedErrorRate, averageResponseTime, recoveryTime } = params;
...
class UnreliableFileSystem {
  ...
  constructor(failureRate: number = 0.1, failureType: 'read' | 'write' | 'delete' | 'permission' | 'disk_full' = 'read') {
  ...
  async writeFile(filePath: string, data: string): Promise<void> {
    if (Math.random() < this.failureRate) {
      switch (this.failureType) {
        case 'write':
          throw new Error(`ENOSPC: no space left on device, write '${filePath}'`);
        case 'permission':
...
describe('Reliability and Stability Testing', () => {
  ...
  describe('Basic Stability Testing', () => {
    ...
    it('should handle intermittent failures gracefully', async () => {
      ...
      expect(result.successfulOperations).toBeGreaterThan(result.failedOperations);
    }, 10000);
  ...
  describe('Resource Management and Isolation', () => {
    ...
    it('should handle resource contention gracefully', async () => {
      ...
      expect(result.stabilityScore).toBeGreaterThanOrEqual(60);
      expect(sharedResource.accessCount).toBeGreaterThan(result.successfulOperations);
    }, 10000);
    ...
    it('should cleanup resources properly after test completion', async () => {
      ...
      const operation = async () => {
        const resource = { id: `resource_${Date.now()}_${Math.random()}`, active: true };
  ...
  describe('Long-term Stability Testing', () => {
    it('should maintain performance over extended periods', async () => {
      ...
      console.log(`Performance consistency: ${degradation.toFixed(2)}x, operations: ${result.totalOperations}`);
    }, 15000);
    ...
    it('should detect memory leaks over time', async () => {
      ...
      console.log(`Potential memory leaks detected: ${leaks.potentialLeaks.length}`);
    }, 10000);
  });
