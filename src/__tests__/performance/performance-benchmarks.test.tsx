  ...
  describe('Performance Utilities', () => {
    ...
    it('should benchmark PerformanceUtils memory checks', async () => {
      const result = await benchmark(
        ...
        () => {
          const shouldSkip = PerformanceUtils.shouldSkipRender(Date.now() - 10, 60);
    ...
    it('should benchmark performance hooks', async () => {
      const TestComponentWithPerf = () => {
        ...
        React.useEffect(() => {
          ...
          // Simulate some work
          const result = Array(100).fill(0).reduce((a, b) => a + Math.random(), 0);
  ...
  describe('Performance Regression Detection', () => {
    it('should detect significant performance regressions', async () => {
      ...
      if (baselineResult.averageTime > expectedBaseline * 1.5) {
        console.warn(`Performance regression detected: ${baselineResult.averageTime}ms vs expected ${expectedBaseline}ms`);
      }
    ...
    it('should maintain 60fps target for rendering operations', async () => {
      ...
      console.log(`Frame time utilization: ${(result.averageTime / targetFrameTime * 100).toFixed(1)}%`);
    });
  });
      ...
      const result = await benchmark(
        ...
        () => {
          ...
          const messages = [
            ...
            { id: '2', content: 'Hi there! How can I help?', role: 'assistant' as const }
          ];
    ...
    it('should maintain performance under memory pressure', async () => {
      const result = await benchmark(
        ...
        () => {
          ...
          const largeData = Array(1000).fill(0).map((_, i) => `Large data item ${i}`);
