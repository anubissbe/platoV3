  ...
  describe("MemoryManager", () => {
    describe("initialization", () => {
      test("should create memory directory if it does not exist", async () => {
        ...
        expect(mockFs.mkdir).toHaveBeenCalledWith(testMemoryDir, {
          recursive: true,
        });
      });
  ...
  describe("cost metadata integration", () => {
    test("should save memory entries with cost metadata", async () => {
      ...
      const writeCall = (mockFs.writeFile as jest.Mock).mock.calls.find(
        (call: any[]) => call[0].includes("test-with-cost-123.json"),
      );
    ...
    test("should update memory cost metadata for existing entries", async () => {
      ...
      expect(lastWriteCall[0]).toEqual(
    ...
    test("should calculate total costs for date range", async () => {
      ...
      const costSummary = await manager.getTotalCostForPeriod(
        startDate,
        endDate,
      );
    ...
    test("should get memories with cost metadata for specific session", async () => {
      ...
      const sessionAMemories =
        await manager.getSessionMemoriesWithCost("session-a");
      expect(sessionAMemories).toHaveLength(2);
      expect(
        sessionAMemories.every(
          (m) => m.costMetadata?.sessionId === "session-a",
        ),
      ).toBe(true);

      const sessionBMemories =
        await manager.getSessionMemoriesWithCost("session-b");
    ...
    test("should update and retrieve session cost analytics", async () => {
      ...
      const costData = {
        ...
        modelBreakdown: {
          ...
          "gpt-4": { cost: 0.02, tokens: 1000, interactions: 2 },
        },
      };

      await manager.updateSessionCostAnalytics(sessionId, costData);

      const retrievedAnalytics =
        await manager.getSessionCostAnalytics(sessionId);
      ...
      expect(retrievedAnalytics!.modelBreakdown).toEqual(
    ...
    test("should handle session with no cost data gracefully", async () => {
      ...
      const analytics = await manager.getSessionCostAnalytics(
