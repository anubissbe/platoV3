...
describe("permissions", () => {
  ...
  beforeEach(() => {
    ...
    (YAML.stringify as jest.Mock).mockImplementation((obj: any) =>
      JSON.stringify(obj),
    );
  ...
  describe("loadPermissions", () => {
    it("should load and merge permissions from global and project configs", async () => {
      const globalConfig = {
        permissions: { defaults: { fs_patch: "confirm" } },
      };
      const projectConfig = {
        permissions: {
  ...
  describe("checkPermission", () => {
      ...
      mockFs.readFile.mockResolvedValue(
        JSON.stringify({
          permissions: {
            ...
            rules: [{ match: { tool: "other_tool" }, action: "deny" }],
          },
        }),
      );
    ...
    it("should match glob patterns correctly", async () => {
      ...
      mockFs.readFile.mockResolvedValue(
        JSON.stringify({
          permissions: {
            rules: [{ match: { path: "/test/**" }, action: "confirm" }],
          },
        }),
      );
    ...
    it("should match command patterns with regex", async () => {
      ...
      mockFs.readFile.mockResolvedValue(
        JSON.stringify({
          permissions: {
            rules: [{ match: { command: "rm.*" }, action: "deny" }],
          },
        }),
      );
  ...
  describe("getProjectPermissions", () => {
    it("should return project permissions", async () => {
      const config = {
        permissions: { defaults: { fs_patch: "confirm" }, rules: [] },
      };
      const mockFs = fs as jest.Mocked<typeof fs>;
