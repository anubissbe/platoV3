...
describe("Command Test Framework", () => {
  ...
  describe("Mock Infrastructure", () => {
    test("should have all required mocks initialized", () => {
      expect(
        jest.isMockFunction(
          require("../../providers/chat_fallback").chatCompletions,
        ),
      ).toBe(true);
      expect(jest.isMockFunction(require("../../tools/patch").apply)).toBe(
        true,
      );
      expect(
        jest.isMockFunction(require("../../policies/security").reviewPatch),
      ).toBe(true);
      expect(
        jest.isMockFunction(require("../../tools/permissions").checkPermission),
      ).toBe(true);
      expect(jest.isMockFunction(require("../../config").setConfigValue)).toBe(
        true,
      );
    });
