...
describe("Session Persistence Integration", () => {
  ...
  describe("Context State Serialization", () => {
    test("should serialize context state with all components", async () => {
      ...
      await index.addFile({
        path: "/test/file1.ts",
        symbols: [
          { name: "TestClass", type: "class" as any, line: 1, exported: true },
        ],
      ...
      const serialized =
        await persistenceManager.serializeContextState(contextState);
    ...
    test("should handle serialization of empty context state", async () => {
      ...
      const serialized =
        await persistenceManager.serializeContextState(emptyState);
    ...
    test("should include context configuration in serialization", async () => {
      ...
      expect(serialized.userPreferences).toHaveProperty(
        "enableAutoRefresh",
        true,
      );
      expect(serialized.userPreferences).toHaveProperty(
      ...
      expect(serialized.userPreferences.excludePatterns).toEqual([
        "*.test.ts",
        "*.spec.js",
      ]);
    });
  ...
  describe("Context State Deserialization", () => {
    test("should deserialize context state correctly", async () => {
      ...
      const deserialized =
        await persistenceManager.deserializeContextState(serializedState);
      ...
      expect(deserialized.currentFiles).toEqual([
        "/test/file1.ts",
        "/test/file2.ts",
      ]);
    ...
    test("should handle missing or corrupted serialized data", async () => {
      ...
      const deserialized =
        await persistenceManager.deserializeContextState(corruptedState);
    ...
    test("should maintain version compatibility", async () => {
      ...
      const deserialized =
        await persistenceManager.deserializeContextState(oldVersionState);
  ...
  describe("Session.json Integration", () => {
    ...
    test("should merge session data with existing orchestrator data", async () => {
      ...
      const merged = await persistenceManager.mergeWithExistingSession(
        contextState,
        existingSessionData,
      );
      ...
      expect(merged.contextManager.currentFiles).toEqual([
        "/test/merge-file.ts",
      ]);
    });
  });
  ...
  describe("Memory Manager Integration", () => {
    test("should save context state to memory system", async () => {
      ...
      await persistenceManager.saveToMemory(
        contextState,
    ...
    test("should load context state from memory system", async () => {
      ...
      const loaded = await persistenceManager.loadFromMemory(
        "test-memory-context",
      );
    ...
    test("should create context history entries", async () => {
      ...
      await persistenceManager.createHistorySnapshot(
        contextState,
        "manual-save",
        "User saved context",
      );
  ...
  describe("Smart Context Resumption", () => {
    test("should restore context state with intelligent merging", async () => {
      ...
      const merged = await persistenceManager.smartResume(
        savedState,
        currentState,
      );

      // Should merge intelligently
      expect(merged.currentFiles).toEqual([
        "/test/saved-file.ts",
        "/test/current-file.ts",
      ]);
    ...
    test("should validate restored context state", async () => {
      ...
      const isInvalid = await persistenceManager.validateContextState(
        invalidState as any,
      );
  ...
  describe("Export/Import Functionality", () => {
    test("should export context configuration", async () => {
      ...
      const exported =
        await persistenceManager.exportConfiguration(contextState);
    ...
    test("should handle import validation errors", async () => {
      ...
      await expect(
        persistenceManager.importConfiguration(invalidImportData as any),
  ...
  describe("Error Handling and Recovery", () => {
    test("should handle disk I/O errors gracefully with fallback", async () => {
      ...
      await expect(
        persistenceManager.saveToSession(contextState),
    ...
    test("should throw error when both session and memory fallback fail", async () => {
      ...
      mockMemoryManager.addMemory.mockRejectedValue(
      ...
      await expect(
        testPersistenceManager.saveToSession(contextState),
    ...
    test("should provide fallback when memory system is unavailable", async () => {
      ...
      await expect(
        persistenceManager.saveToMemory(contextState, "test-fallback"),
      ).resolves.not.toThrow();
    });
  });
});
