...
export class ContentSampler {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!inClass && classPattern.test(line)) {
        ...
        const isMethodSignature =
          /(?:public|private|protected|async|static)?\s*(?:async\s+)?(\w+)\s*\([^)]*\)\s*[:{]/.test(
            line,
        ...
        if (
          isMethodSignature &&
          !inMethod &&
          methodCount < maxMethodsPerClass
        ) {
          methodCount++;
          inMethod = true;
          methodDepth = 0;
  ...
  private calculateTokenAllocations(
    ...
    const totalRelevance = files.reduce(
      (sum, f) => sum + (f.relevanceScore || 50),
      0,
    );
