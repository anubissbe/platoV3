...
export class AuditIndexer extends EventEmitter {
  ...
  constructor(options: IndexOptions = {}) {
    ...
    this.options = {
      indexDirectory:
        ...
        path.join(process.cwd(), ".plato", "audit-indexes"),
  ...
  async getEntry(indexEntry: IndexEntry): Promise<AuditEntry | null> {
    // Try cache first
    if (
      this.options.enableInMemoryCache &&
      this.entryCache.has(indexEntry.entryId)
    ) {
    ...
    try {
      ...
      const filePath = path.join(
        path.dirname(this.options.indexDirectory),
        "audit-logs",
        indexEntry.fileName,
      );
      ...
      for (
        let i = Math.max(0, indexEntry.position - 10);
        i < Math.min(lines.length, indexEntry.position + 10);
        i++
      ) {
   ...
   */
  getStats(): IndexStats {
    const totalEntries = Array.from(this.indexes.values()).reduce(
      (sum, index) =>
        sum +
        Array.from(index.values()).reduce(
    ...
    const avgQueryTime =
      this.performanceMetrics.query_times.length > 0
        ? this.performanceMetrics.query_times.reduce(
            (sum, time) => sum + time,
            0,
          ) / this.performanceMetrics.query_times.length
        : 0;
    ...
    try {
      ...
      for (const [indexName, index] of this.indexes) {
        for (const [key, entries] of index) {
          ...
          uniqueEntries.sort(
            (a, b) => a.indexed_at.getTime() - b.indexed_at.getTime(),
          );
  ...
  private addToCache(entry: AuditEntry): void {
    if (this.entryCache.size >= this.options.cacheSize) {
      ...
      const keysToRemove = Array.from(this.entryCache.keys()).slice(
        0,
        Math.floor(this.options.cacheSize * 0.1),
      );
  ...
  ): Array<{ index: string; key: string }> {
    const queries: Array<{ index: string; key: string }> = [];

    if (criteria.tool) queries.push({ index: "tool", key: criteria.tool });
    if (criteria.action)
      queries.push({ index: "action", key: criteria.action });
    if (criteria.level) queries.push({ index: "level", key: criteria.level });
    if (criteria.category)
      queries.push({ index: "category", key: criteria.category });
    if (criteria.source)
      queries.push({ index: "source", key: criteria.source });
    if (criteria.profile)
      queries.push({ index: "profile", key: criteria.profile });
    if (criteria.sessionId)
      queries.push({ index: "session_id", key: criteria.sessionId });
    if (criteria.git_branch)
      queries.push({ index: "git_branch", key: criteria.git_branch });
  ...
  private intersectSearchResults(
    queries: Array<{ index: string; key: string }>,
  ): IndexEntry[] {
    ...
    for (let i = 1; i < queries.length; i++) {
      const queryResults = this.getIndexEntries(
        queries[i].index,
        queries[i].key,
      );
  ...
  private applyPostIndexFiltering(
    results: IndexEntry[],
    criteria: AuditSearchCriteria,
  ): IndexEntry[] {
    ...
    if (
      ...
      criteria.has_user_decision !== undefined ||
  ...
  private trackQueryPerformance(startTime: number): void {
    ...
    if (this.performanceMetrics.query_times.length > 1000) {
      this.performanceMetrics.query_times =
        this.performanceMetrics.query_times.slice(-1000);
    }
  }
