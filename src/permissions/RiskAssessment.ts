...
export class RiskAssessment {
  ...
  private static readonly DANGEROUS_OPERATIONS = {
    ...
    database: {
      base: "high",
      patterns: ["DROP", "TRUNCATE", "DELETE", "ALTER"],
    },
  };
  ...
  static assessRisk(query: PermissionQuery): {
    ...
    if (query.metadata?.affectedFiles && query.metadata.affectedFiles > 10) {
      score += 20;
      factors.push(
        `Batch operation affecting ${query.metadata.affectedFiles} files`,
      );
    }
  ...
  private static assessPathRisk(filePath: string): {
    ...
    if (
      /\.(conf|config|cfg|ini|env|json|yaml|yml|toml)$/i.test(normalizedPath)
    ) {
      score += 15;
   ...
   */
  private static assessOperationRisk(
    tool: string,
    operation: string,
  ): {
    score: number;
    ...
    const dangerousOp =
      this.DANGEROUS_OPERATIONS[key as keyof typeof this.DANGEROUS_OPERATIONS];
