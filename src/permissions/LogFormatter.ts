...
export class JsonLogFormatter implements LogFormatter {
  ...
  format(entry: AuditEntry): string {
    ...
    return JSON.stringify(formattedEntry, null, this.options.format === 'json' ? 0 : 2);
  }
  ...
  private formatTimestamp(date: Date): string {
    ...
    return this.options.date_format === 'iso' ? date.toISOString() : date.toLocaleString();
  }
}
...
export class StructuredLogFormatter implements LogFormatter {
  ...
  format(entry: AuditEntry): string {
    ...
    if (this.options.include_metadata && entry.metadata) {
      if (entry.metadata.duration_ms) parts.push(`DURATION=${entry.metadata.duration_ms}ms`);
      if (entry.metadata.risk_score !== undefined) parts.push(`RISK=${entry.metadata.risk_score}`);
      if (entry.metadata.cache_hit !== undefined) parts.push(`CACHE_HIT=${entry.metadata.cache_hit}`);
...
export class CsvLogFormatter implements LogFormatter {
  private readonly headers = [
    'id', 'timestamp', 'level', 'category', 'tool', 'operation', 'path', 
    'action', 'reason', 'confidence', 'profile', 'session_id', 'user_decision',
    'source', 'git_branch', 'duration_ms', 'risk_score', 'cache_hit', 'tags'
  ];
...
export class HumanReadableLogFormatter implements LogFormatter {
  constructor(private options: LogFormatOptions = { format: 'human-readable' }) {}
  ...
  format(entry: AuditEntry): string {
    ...
    if (entry.user_decision) {
      const decisionEmoji = entry.user_decision === 'approved' ? '✅' : entry.user_decision === 'denied' ? '❌' : '⏭️';
    ...
    if (this.options.include_context && entry.context) {
      ...
      if (entry.context.git_context) {
        ...
        if (entry.context.git_context.commit_hash) {
          lines.push(`    Commit: ${entry.context.git_context.commit_hash.substring(0, 8)}`);
        }
        lines.push(`    Clean Working Dir: ${entry.context.git_context.is_clean ? 'Yes' : 'No'}`);
      }
    ...
    if (this.options.include_metadata && entry.metadata) {
      ...
      if (entry.metadata.risk_score !== undefined) {
        const riskEmoji = entry.metadata.risk_score < 33 ? '🟢' : entry.metadata.risk_score < 67 ? '🟡' : '🔴';
        lines.push(`    ${riskEmoji} Risk Score: ${entry.metadata.risk_score}/100`);
      }
      ...
      if (entry.metadata.compliance_flags && entry.metadata.compliance_flags.length > 0) {
        lines.push(`    Compliance: ${entry.metadata.compliance_flags.join(', ')}`);
      }
    }
 ...
 */
export function createAuditMetadata(level: 'debug' | 'info' | 'warn' | 'error' = 'info'): import('./types').AuditMetadata {
  return {
    version: '1.0.0',
    level,
    category: 'permission',
