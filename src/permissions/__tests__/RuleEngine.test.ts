...
describe("RuleEngine", () => {
      ...
      const testCases = [
        {
          ...
          expectedAction: "allow" as PermissionAction,
        },
        {
        ...
        },
        {
          ...
          expectedAction: "deny" as PermissionAction,
        },
      ...
      const rules: Rule[] = [
        {
          match: {
            tool: "fs_write",
            path: "src/**/*.{ts,js}|tests/**/*.spec.{ts,js}",
          },
          action: "allow",
          priority: 100,
  ...
  describe("Rule Validation", () => {
    ...
    it("should detect invalid path patterns", () => {
      ...
      expect(
        validation.errors.some((error) =>
          error.includes("Invalid characters in path pattern"),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Default Action Resolution", () => {
    it("should use default action when no rules match", async () => {
      ...
      expect(result.reason).toContain(
        "No specific rule found, using default: deny",
      );
    });
    ...
    it("should support pattern matching for default tool selection", async () => {
      ...
      expect(result.action).toBe("allow");
      expect(result.reason).toContain(
        "No specific rule found, using default: allow",
      );
    });
  });
});
