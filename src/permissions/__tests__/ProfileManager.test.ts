...
import { Profile, ProfileActivationRule, PermissionProfile, Rule } from '../types';
...
describe('ProfileManager', () => {
  ...
  describe('Profile Loading', () => {
    ...
    it('should merge global and project configurations', async () => {
      const globalConfig = {
        permissions: {
          profiles: {
            development: { description: 'Global dev', activation: {}, defaults: {}, rules: [] }
          }
        }
      };
      const projectConfig = {
        permissions: {
          profiles: {
            development: { description: 'Project dev', activation: {}, defaults: { fs_write: 'allow' }, rules: [] },
            testing: { description: 'Test profile', activation: {}, defaults: {}, rules: [] }
          }
        }
      };
  ...
  describe('Context Detection', () => {
    ...
    it('should detect active profile based on git branch', async () => {
      ...
      expect(mockExeca).toHaveBeenCalledWith('git', ['branch', '--show-current'], { cwd: mockCwd });
  ...
  describe('Profile Inheritance', () => {
    beforeEach(async () => {
      const config = {
        permissions: {
          global_rules: [
            { match: { tool: 'fs_write', path: '**/.env*' }, action: 'deny', priority: 100 }
          ],
          profiles: {
            development: {
              description: 'Development profile',
              ...
              rules: [
                { match: { tool: 'fs_write', path: 'src/**' }, action: 'allow', priority: 50 }
              ]
            }
          }
        }
    ...
    it('should support full inheritance chain: global → project → profile → temporary', async () => {
      ...
      const fullConfig = {
        permissions: {
          global_rules: [
            { match: { tool: 'fs_write' }, action: 'confirm', priority: 10, reason: 'Global rule' }
          ],
          project_rules: [
            { match: { tool: 'exec' }, action: 'deny', priority: 20, reason: 'Project rule' }
          ],
          profiles: {
            development: {
              ...
              defaults: {},
              rules: [
                { match: { tool: 'fs_read' }, action: 'allow', priority: 30, reason: 'Profile rule' }
              ]
            }
          }
        }
        ...
        permissions: {
          profiles: {
            base: {
              ...
              rules: [
                { match: { tool: 'base_tool' }, action: 'allow', priority: 10, reason: 'Base rule' }
              ]
            },
            derived: {
              ...
              inherits_from: 'base',
              rules: [
                { match: { tool: 'derived_tool' }, action: 'deny', priority: 20, reason: 'Derived rule' }
              ]
            }
          }
        }
  ...
  describe('Profile Validation', () => {
    it('should validate profile configuration structure', async () => {
      ...
      await expect(profileManager.loadProfiles()).rejects.toThrow('Profile must have a description');
    });
    ...
    it('should validate activation rules', async () => {
      ...
      await expect(profileManager.loadProfiles()).rejects.toThrow('Invalid branch pattern');
    });
  });
  ...
  describe('Temporary Rule Management', () => {
    ...
    it('should remove specific temporary rules', () => {
      const rule1: Rule = { id: 'temp-1', match: { tool: 'tool1' }, action: 'allow' };
      const rule2: Rule = { id: 'temp-2', match: { tool: 'tool2' }, action: 'deny' };
    ...
    it('should clear all temporary rules', () => {
      profileManager.addTemporaryRule({ id: 'temp-1', match: { tool: 'tool1' }, action: 'allow' });
      profileManager.addTemporaryRule({ id: 'temp-2', match: { tool: 'tool2' }, action: 'deny' });
    ...
    it('should emit events for temporary rule changes', () => {
      ...
      const rule: Rule = { id: 'temp-test', match: { tool: 'test' }, action: 'allow' };
