...
export const PermissionPrompt: React.FC<PermissionPromptProps> = ({
  ...
  if (!query) {
    return (
      <Box flexDirection="column" borderStyle="round" borderColor="red" padding={1}>
        <Text color="red" bold>Invalid permission request</Text>
  ...
  const handleResponse = useCallback((action: 'allow' | 'deny', permanent: boolean = false, elevate?: boolean, duration?: number) => {
  ...
  useInput((input, key) => {
    ...
    if (showElevationMenu) {
      if (key.escape) {
        ...
        setSelectedOption(Math.min(elevationOptions.length - 1, selectedOption + 1));
      ...
      <Box flexDirection="column" borderStyle="round" borderColor="cyan" padding={1}>
        ...
        <Box flexDirection="column" marginTop={1}>
          {elevationOptions.map((option, index) => (
            <Text key={index} color={selectedOption === index ? 'cyan' : undefined}>
              {selectedOption === index ? '▶ ' : '  '}{option.label}
            </Text>
          ))}
          {allowCustomDuration && (
            <Text color={selectedOption === elevationOptions.length ? 'cyan' : undefined}>
              {selectedOption === elevationOptions.length ? '▶ ' : '  '}[C] Custom duration
            </Text>
          )}
        </Box>
        <Box paddingTop={1}>
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={riskColor} padding={1}>
      ...
      {currentElevation?.active && (
        <Box marginBottom={1}>
          <Text color="cyan">
            ⚡ Elevated permissions active ({Math.floor(currentElevation.remainingTime / 60000)} minutes remaining)
          </Text>
        </Box>
      )}
      ...
      <Box flexDirection="column" marginBottom={1}>
        ...
        {query.path && (
          <Text>
            <Text bold>Path: </Text>
            <Text color={riskLevel === 'critical' ? 'red' : 'white'}>{query.path}</Text>
          </Text>
        )}
        {affectedFiles && (
          <Text>
      ...
      {matchedRule && (
        <Box flexDirection="column" marginBottom={1}>
          ...
          {conflictingRules && conflictingRules.length > 0 && (
            <Text dimColor>  Overrides {conflictingRules.length} lower priority rule(s)</Text>
          )}
        </Box>
      )}
      ...
      {riskLevel === 'critical' && (
        <Box marginBottom={1} borderStyle="double" borderColor="red" padding={1}>
          <Text color="red" bold>
