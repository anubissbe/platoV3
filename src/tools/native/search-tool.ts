...
export class SearchTool extends EventEmitter implements NativeTool {
  ...
  async execute(args: Record<string, any>): Promise<SearchResult> {
    ...
    try {
      ...
      const searchPath = await this.validateAndNormalizePath(
        searchArgs.path,
        searchArgs.pattern,
      );
      ...
      const truncated = searchArgs.maxResults
        ? matches.length >= searchArgs.maxResults &&
          (actualTotalMatches || totalMatches) > matches.length
        : false;
  ...
  async *stream(args: Record<string, any>): AsyncGenerator<ToolEvent> {
    ...
    try {
      ...
      for await (const match of this.streamRipgrepResults(
        ripgrepArgs,
        searchArgs.maxResults,
      )) {
        matches.push(match);
  ...
  private async *streamRipgrepResults(
    ripgrepArgs: string[],
    maxResults?: number,
