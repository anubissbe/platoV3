...
describe("Security and Resource Management", () => {
  ...
  describe("Workspace Sandboxing", () => {
    ...
    test("should prevent parent directory traversal attacks", async () => {
      ...
      const result =
        await securityManager.validateWorkspaceAccess(maliciousPath);
    ...
    test("should validate workspace boundaries with complex paths", async () => {
      ...
      for (const testCase of testCases) {
        const result = await securityManager.validateWorkspaceAccess(
          testCase.path,
        );
  ...
  describe("Concurrency Limits and Rate Limiting", () => {
    test("should enforce maximum concurrent operations", async () => {
      ...
      // Try to exceed limit
      const excessResult =
        await resourceManager.acquireResource("excess-operation");
    ...
    test("should implement rate limiting per tool type", async () => {
      ...
      for (let i = 0; i < rateLimit.requests; i++) {
        const result = await resourceManager.checkRateLimit(
          "read",
          "test-client",
        );
        expect(result.allowed).toBe(true);
      ...
      // Exceed rate limit
      const exceededResult = await resourceManager.checkRateLimit(
        "read",
        "test-client",
      );
    ...
    test("should handle resource cleanup on timeout", async () => {
      ...
      const newAcquisition =
        await resourceManager.acquireResource("after-timeout");
  ...
  describe("Resource Monitoring", () => {
    ...
    test("should track file handle usage", async () => {
      ...
      expect(afterCloseHandles.count).toBeLessThanOrEqual(
        afterOpenHandles.count,
      );
    });
  ...
  describe("Telemetry Events", () => {
    test("should emit telemetry with Claude Code-compatible fields", async () => {
      ...
      await securityManager.validateWorkspaceAccess(
    ...
    test("should emit security violation telemetry", async () => {
      ...
      securityManager.on("security-violation", (event) =>
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    test("should handle resource exhaustion gracefully", async () => {
      ...
      const secondResult =
        await resourceManager.acquireResource("second-operation");
  ...
  describe("Integration with Existing Tools", () => {
    test("should integrate with ReadTool security validation", async () => {
      ...
      const securityResult = await securityManager.validateFileAccess(
        testFile,
        "read",
      );
