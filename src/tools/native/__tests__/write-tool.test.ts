...
describe("WriteTool", () => {
  ...
  describe("Permission Preservation", () => {
    ...
    it("should apply specified permissions to new files", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.bytesWritten).toBe(
        Buffer.byteLength("content with perms", "utf8"),
      );
      expect(result.encoding).toBe("utf8");
    });
  });
  ...
  describe("Streaming and Performance", () => {
    ...
    it("should handle concurrent writes to different files", async () => {
      ...
      results.forEach((result, i) => {
        expect(result.success).toBe(true);
        expect(result.bytesWritten).toBe(
          Buffer.byteLength(`Content for file ${i}`),
        );
      });
    });
    ...
    it("should handle write conflicts appropriately", async () => {
      ...
      expect(results[0].bytesWritten).toBe(
        Buffer.byteLength("Content A", "utf8"),
      );
      expect(results[1].bytesWritten).toBe(
        Buffer.byteLength("Content B", "utf8"),
      );
    });
  });
