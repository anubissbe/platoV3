...
describe("Directory Tools", () => {
  ...
  afterEach(async () => {
    if (tempDir) {
      try {
        ...
        await (
          typeof (fs as any).rm === "function" ? (fs as any).rm : fs.rmdir
  ...
  describe("MoveTool", () => {
    ...
    beforeEach(async () => {
      ...
      await fs.writeFile(
        path.join(tempDir, "sourcedir", "nested.txt"),
        "Nested content",
      );
    ...
    it("should preserve metadata when requested", async () => {
      ...
      if (process.platform !== "win32") {
        const stats = await fs.stat(destPath);
        expect(
          Math.abs(stats.mtime.getTime() - testTime.getTime()),
        ).toBeLessThan(1000);
      }
    });
