...
describe("ListTool", () => {
  ...
  describe("Recursive Listing", () => {
    it("should list files recursively", async () => {
      ...
      expect(filePaths.some((p) => p.includes("subdir1/nested.txt"))).toBe(
        true,
      );
      expect(filePaths.some((p) => p.includes("subdir1/script.py"))).toBe(true);
      expect(
        filePaths.some((p) => p.includes("subdir2/deep/very/deep.txt")),
      ).toBe(true);
    });
  ...
  describe("Sorting and Ordering", () => {
    ...
    it("should sort by modification time", async () => {
      ...
      const files = result.files!.filter(
        (f) => f.name.includes(".txt") && f.name !== "file1.txt",
      );
  ...
  describe("File Statistics", () => {
    ...
    it("should calculate total size correctly", async () => {
      ...
      const calculatedSize = result.files!.reduce(
  ...
  describe("Performance and Large Directories", () => {
    ...
    it("should stream results for very large directories", async () => {
      ...
      const promises = Array.from({ length: 100 }, (_, i) =>
        fs.writeFile(
          path.join(largeDirPath, `stream-file${i}.txt`),
          `Content ${i}`,
        ),
      );
      await Promise.all(promises);
