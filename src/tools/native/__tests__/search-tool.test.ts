...
} from "../../__tests__/helpers/test-reliability";
...
describe("SearchTool", () => {
  ...
  beforeEach(async () => {
    ...
    const testFiles = {
      ...
      "binary.dat": Buffer.from([
        0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
      ]).toString("binary"),
      "subdir/nested.txt":
        "Nested file content\nWith nested patterns\nMore nested content",
      "subdir/deep/very/deep.txt":
        "Very deep content\nHELLO from deep file\nEnd of deep content",
      "subdir/script.sh":
        '#!/bin/bash\necho "Hello from script"\ngrep "pattern" file.txt',
      "large.txt": Array.from(
        { length: 1000 },
        (_, i) => `Line ${i + 1}: Pattern ${i % 10}`,
      ).join("\n"),
    };
    ...
    for (const [filePath, content] of Object.entries(testFiles)) {
      ...
      if (dirPath && dirPath !== tempDir) {
        await withRetry(() => fs.mkdir(dirPath, { recursive: true }), {
          maxRetries: 3,
          delayMs: 50,
        });
      }
  ...
  describe("Regular Expression Search", () => {
    ...
    it("should support complex regex patterns", async () => {
      ...
      const jsonMatches = result.matches!.filter((m) =>
  ...
  describe("Binary File Handling", () => {
    it("should skip binary files by default", async () => {
      ...
      const binaryMatches = result.matches!.filter((m) =>
        m.file.endsWith(".dat"),
      );
  ...
  describe("Security and Path Validation", () => {
    ...
    it("should handle permission denied gracefully", async () => {
      if (process.platform !== "win32") {
        ...
        await fs.writeFile(
          path.join(restrictedDir, "secret.txt"),
          "secret content",
        );
