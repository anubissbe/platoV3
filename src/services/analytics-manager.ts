...
export class AnalyticsManager {
  ...
  private summaryCache: Map<
    string,
    { summary: AnalyticsSummary; timestamp: number }
  >;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to initialize AnalyticsManager: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async getSummary(
    ...
    // Generate summary
    const summary = this.generateSummaryFromMetrics(
      metrics,
      period,
      startDate,
      endDate,
  ...
  async exportData(
    ...
    if (duration > 2000) {
      console.warn(
        `Export took ${duration}ms - exceeds 2s performance requirement`,
      );
    }
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `Failed to read analytics directory ${this.options.dataDir}:`,
          error,
        );
      ...
      const dataFiles = files.filter(
        (file) => file.endsWith(".json") && file !== "analytics-index.json",
      );
  ...
  async compact(): Promise<void> {
    ...
    try {
      ...
      for (const fileName of dataFiles) {
        ...
        try {
          ...
          if (compactedMetrics.length < dataFile.metrics.length) {
            ...
            dataFile.metadata.totalCost = compactedMetrics.reduce(
              (sum, m) => sum + m.cost,
              0,
            );
            ...
            console.log(
              `Compacted ${fileName}: removed ${dataFile.metrics.length - compactedMetrics.length} duplicates, saved ${savings} bytes`,
            );
          }
        } catch (error) {
      ...
      if (totalSavings > 0) {
        await this.rebuildIndex();
        console.log(
          `Data compaction completed. Total space saved: ${totalSavings} bytes`,
        );
      } else {
      ...
      console.error(
        `Data compaction failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private validateMetric(metric: CostMetric): void {
    ...
    if (new Date(metric.timestamp).getTime() > Date.now() + 60000) {
  ...
  private adjustIntelligentBatchSize(): void {
    ...
    if (avgTime > targetTime * 1.5) {
      ...
      this.intelligentBatchSize = Math.max(
        10,
        Math.floor(this.intelligentBatchSize * 0.8),
      );
    } else if (avgTime < targetTime * 0.5) {
      // Writes are very fast, increase batch size for efficiency
      this.intelligentBatchSize = Math.min(
        this.options.batchSize * 2,
        Math.floor(this.intelligentBatchSize * 1.2),
      );
    }
  }
  ...
  private scheduleBatchFlush(): void {
    ...
    const timeRemaining =
      this.options.maxBatchWaitTime - (Date.now() - this.lastBatchFlush);
  ...
  private async flushPendingMetricsWithStats(): Promise<void> {
    ...
    this.batchWriteStats.averageTime =
      this.batchWriteStats.totalTime / this.batchWriteStats.totalWrites;
    this.lastBatchFlush = Date.now();
  }
      ...
      for (const fileName of dataFiles) {
        ...
        try {
          ...
          if (!dataFile.metrics || !Array.isArray(dataFile.metrics)) {
            console.warn(
              `File ${fileName} has invalid metrics array, skipping`,
            );
            continue;
          }
          ...
          if (dataFile.metrics.length > 0) {
            ...
            this.indexCache.files[month] = {
              ...
              totalCost:
                dataFile.metadata.totalCost ||
                dataFile.metrics.reduce((sum, m) => sum + m.cost, 0),
  ...
  private async appendMetricsToFile(
    ...
    existingData.metadata.totalCost = existingData.metrics.reduce(
      (sum, m) => sum + m.cost,
      0,
    );
  ...
  private generateSummaryFromMetrics(
    ...
    for (const [sessionId, cost] of Object.entries(sessionCosts)) {
      if (cost > maxCost) {
        ...
        const tokens = sessionMetrics.reduce(
          (sum, m) => sum + m.inputTokens + m.outputTokens,
          0,
        );
    ...
    return {
      ...
      endDate:
        typeof endDate === "number" ? endDate : (endDate as Date).getTime(),
      dateRange: {
      ...
      avgCostPerSession:
        uniqueSessions > 0
          ? Math.round((totalCost / uniqueSessions) * 100) / 100
          : 0,
      costByProvider,
      costByModel,
      mostExpensiveSession,
      modelBreakdown: Object.keys(costByModel).reduce(
        (acc, model) => {
          acc[model] = {
    ...
    for (const metric of metrics) {
      const row = [
        typeof metric.timestamp === "number"
          ...
          : (metric.timestamp as Date).toISOString(),
