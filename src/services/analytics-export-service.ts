...
export class AnalyticsExportService extends EventEmitter {
  ...
  private async processMetrics(
    ...
    if (options.dateRange) {
      filteredMetrics = metrics.filter((m) => {
        const metricDate =
          typeof m.timestamp === "number" ? new Date(m.timestamp) : m.timestamp;
        return (
          metricDate >= options.dateRange!.start &&
          metricDate <= options.dateRange!.end
        );
      });
    }
    ...
    for (let i = 0; i < filteredMetrics.length; i += batchSize) {
      ...
      const batch = filteredMetrics.slice(
        i,
        Math.min(i + batchSize, filteredMetrics.length),
      );
      const batchResult =
        options.format === "csv"
   ...
   */
  private generateCompleteJSON(
    metrics: CostMetric[],
    dateRange?: DateRange,
  ): string {
    const totalCost = metrics.reduce((sum, m) => sum + m.cost, 0);
    const totalTokens = metrics.reduce(
      (sum, m) => sum + (m.totalTokens || m.inputTokens + m.outputTokens),
      0,
    );
    ...
    return JSON.stringify(
      {
        ...
        summary: {
          ...
          avgCostPerRecord:
            metrics.length > 0
              ? Math.round((totalCost / metrics.length) * 1000000) / 1000000
              : 0,
        },
        metrics,
      },
  ...
  private async generateExportFile(
    ...
    const filePath =
      options.outputPath || path.join(".plato", "exports", defaultFileName);
