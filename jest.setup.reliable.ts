      ...
      .mockImplementation(async (filePath: string, options?: any) => {
          ...
          if (resolved.startsWith(tempDir)) {
            try {
              ...
              const enhancedError = new Error(
                `readFile failed for ${filePath}: ${error.message}`,
        ...
        if (mockFiles.has(resolved)) {
          ...
          return options && typeof options === "string"
            ? buffer.toString(options as BufferEncoding)
            : buffer;
        }
      ...
      .mockImplementation(async (filePath: string, options?: any) => {
        ...
        for (const tempDir of tempDirs) {
          if (resolved.startsWith(tempDir) || resolved === tempDir) {
            ...
            return original.rm
              ? original.rm(filePath, options)
              : original.rmdir(filePath, options);
          }
        }
...
jest.mock("execa", () => ({
  execaCommand: jest
    .fn()
    .mockImplementation(async (command: string, options?: any) => {
  ...
  execa: jest
    .fn()
    .mockImplementation(
      async (file: string, args?: string[], options?: any) => {
        const timeout = options?.timeout || 30000;
  ...
  async toResolveWithin(received: Promise<any>, timeoutMs: number) {
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(
                new Error(`Promise did not resolve within ${timeoutMs}ms`),
              ),
            timeoutMs,
          ),
        ),
      ...
      return {
        pass: false,
        message: () =>
          `Promise failed to resolve within ${timeoutMs}ms: ${error.message}`,
      };
    }
  },
