...
class PerformanceReporter {
  constructor(globalConfig, options) {
    ...
    this.outputFile =
      this.options.outputFile || "performance-test-results.json";
    this.results = {
  ...
  onTestResult(test, testResult) {
    ...
    if (performanceData.duration > 5000) {
      this.results.summary.slowTests.push({
        ...
        slowTestsCount: performanceData.tests.filter((t) => t.duration > 1000)
          .length,
      });
    }
  }
  ...
  extractPerformanceMetrics(test) {
    ...
    if (test.failureMessages) {
      test.failureMessages.forEach((message) => {
        const opsMatch = message.match(
          /(\d+\.?\d*)\s*operations?\s*per\s*second/i,
        );
        if (opsMatch) {
  ...
  onRunComplete(contexts, results) {
    // Finalize summary
    this.results.summary.averageTestDuration =
      this.results.summary.totalDuration /
      Math.max(1, this.results.summary.totalTests);
  ...
  calculateOverallMetrics() {
    ...
    metrics.benchmarkTests = allTests.filter(
      (t) => t.performanceMetrics?.type === "benchmark",
    ).length;
    metrics.loadTests = allTests.filter(
      (t) => t.performanceMetrics?.type === "load_test",
    ).length;
    metrics.memoryTests = allTests.filter(
      (t) => t.performanceMetrics?.type === "memory_test",
    ).length;
    metrics.stabilityTests = allTests.filter(
      (t) => t.performanceMetrics?.type === "stability_test",
    ).length;

    // Average metrics across all tests
    const testsWithOps = allTests.filter(
      (t) => t.performanceMetrics?.operationsPerSecond,
    );
    if (testsWithOps.length > 0) {
      metrics.averageOperationsPerSecond =
        testsWithOps.reduce(
          (sum, t) => sum + t.performanceMetrics.operationsPerSecond,
          0,
        ) / testsWithOps.length;
    }

    const testsWithResponseTime = allTests.filter(
      (t) => t.performanceMetrics?.averageResponseTime,
    );
    if (testsWithResponseTime.length > 0) {
      metrics.averageResponseTime =
        testsWithResponseTime.reduce(
          (sum, t) => sum + t.performanceMetrics.averageResponseTime,
          0,
        ) / testsWithResponseTime.length;
    }

    // Memory usage statistics
    const memoryUsages = this.results.testResults.map(
      (r) => r.memoryUsage.heapUsed,
    );
    metrics.memoryStats = {
  ...
  printSummary() {
    ...
    console.log(
      `   Average Test Duration: ${summary.averageTestDuration.toFixed(0)}ms`,
    );
    ...
    if (summary.slowTests.length > 0) {
      ...
      summary.slowTests.forEach((test) => {
        console.log(
          `   ${test.testFile}: ${(test.duration / 1000).toFixed(2)}s`,
        );
      });
    }
    ...
    if (summary.performanceMetrics.memoryStats) {
      ...
      console.log(
        `   Average: ${(memStats.average / 1024 / 1024).toFixed(1)}MB`,
      );
    ...
    console.log(`\n🧪 Test Types:`);
    console.log(
      `   Benchmark Tests: ${summary.performanceMetrics.benchmarkTests || 0}`,
    );
    console.log(`   Load Tests: ${summary.performanceMetrics.loadTests || 0}`);
    console.log(
      `   Memory Tests: ${summary.performanceMetrics.memoryTests || 0}`,
    );
    console.log(
      `   Stability Tests: ${summary.performanceMetrics.stabilityTests || 0}`,
    );

    if (summary.performanceMetrics.averageOperationsPerSecond) {
      console.log(`\n⚡ Performance Metrics:`);
      console.log(
        `   Average Operations/Second: ${summary.performanceMetrics.averageOperationsPerSecond.toFixed(1)}`,
      );
    ...
    console.log(
      `   Total Memory: ${(environment.totalMemory / 1024 / 1024 / 1024).toFixed(1)}GB`,
    );
