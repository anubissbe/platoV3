...
module.exports = async () => {
  ...
  process.env.PLATO_TEST_TEMP_DIR = path.join(process.cwd(), 'tmp/plato-performance-test');
  
  // Enable garbage collection exposure for memory testing
  if (typeof global.gc === 'undefined') {
    console.log('⚠️  Garbage collection not exposed. Run with --expose-gc for memory leak detection.');
  } else {
  ...
  const baselineFile = path.join(process.cwd(), 'performance-baselines', 'current-baseline.json');
  ...
  if (process.env.PLATO_MEMORY_MONITORING === 'true') {
    memoryMonitoringInterval = setInterval(() => {
      ...
      const logFile = path.join(process.cwd(), 'performance-logs', 'memory-usage.jsonl');
  ...
  for (const dir of cleanupDirs) {
    ...
    if (fs.existsSync(dirPath)) {
      ...
      if (filesToDelete.length > 0) {
        console.log(`🧹 Cleaned up ${filesToDelete.length} old files from ${dir}`);
      }
    }
  }
