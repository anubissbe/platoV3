  ...
  return {
      ...
      .mockImplementation(async (filePath: string, mode?: number) => {
        if (isRealPath(filePath)) {
          try {
            ...
            const normalizedPath = path.resolve(filePath);
            if (
              mockFiles.has(normalizedPath) ||
              mockStats.has(normalizedPath)
            ) {
              return;
            }
            throw error;
    ...
    chmod: jest
      ...
      .mockImplementation(async (filePath: string, mode: number) => {
        ...
        const error = new Error(
          `ENOENT: no such file or directory, chmod '${filePath}'`,
        ) as NodeJS.ErrnoException;
        error.code = "ENOENT";
        throw error;
      }),
    ...
    rename: jest
      .fn()
      .mockImplementation(async (oldPath: string, newPath: string) => {
    ...
    open: jest
      .fn()
      .mockImplementation(async (filePath: string, flags: string) => {
        if (isRealPath(filePath)) {
    ...
    utimes: jest
      .fn()
      .mockImplementation(
        async (filePath: string, atime: Date, mtime: Date) => {
          if (isRealPath(filePath)) {
          ...
          const error = new Error(
            `ENOENT: no such file or directory, utimes '${filePath}'`,
          ) as NodeJS.ErrnoException;
          error.code = "ENOENT";
          throw error;
        },
...
jest.mock("fs", () => {
  ...
  return {
    ...
    existsSync: jest.fn().mockImplementation((filePath: string) => {
      ...
      const normalizedPath = path.resolve(filePath);
      return (
        mockFiles.has(normalizedPath) || mockDirectories.has(normalizedPath)
      );
    }),
