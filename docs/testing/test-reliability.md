...
await withRetry(() => fs.writeFile(path, content), {
  maxRetries: 3,
  delayMs: 100,
  backoffMultiplier: 2,
});
...
beforeEach(async () => {
  tempDir = await withRetry(() => fs.mkdtemp(path.join(os.tmpdir(), "test-")), {
    maxRetries: 3,
    delayMs: 100,
  });
});
...
test("should process queue items", async () => {
  ...
  await waitForCondition(() => queue.isEmpty(), {
    timeoutMs: 5000,
    intervalMs: 100,
  });
...
afterEach(async () => {
  await testEnv.cleanup();
  if (tempDir) {
    await withRetry(() => fs.rm(tempDir, { recursive: true, force: true }), {
      maxRetries: 3,
      delayMs: 100,
    });
  }
